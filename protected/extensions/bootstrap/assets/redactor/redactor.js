	...
	function ($) {
		...
		Redactor.fn = $.Redactor.prototype = {
			...
			buildEventKeydown: function (e) {
				...
				// pre & down
				if ((
					parent && $(parent).get(0).tagName === 'PRE'
					) || (
					current && $(current).get(0).tagName === 'PRE'
					)) {
					pre = true;
						...
						if (this.opts.linebreaks) {
							...
							if (block && this.opts.rBlockTest.test(block.tagName)) {
								...
								setTimeout($.proxy(function () {
									var blockElem = this.getBlock();
									if ((
										blockElem.tagName === 'DIV' || blockElem.tagName === 'P'
										) && !$(blockElem).hasClass('redactor_editor')) {
			...
			buildEventKeyup: function (e) {
				...
				if (!this.opts.linebreaks && current.nodeType == 3 && (
					parent == false || parent.tagName == 'BODY'
					)) {
				...
				// convert links
				if ((
					this.opts.convertLinks || this.opts.convertImageLinks || this.opts.convertVideoLinks
					) && key === this.keyCode.ENTER) {
			...
			execCommand: function (cmd, param, sync) {
				...
				if (cmd === 'insertunorderedlist' || cmd === 'insertorderedlist') {
					...
					if ($list.length) {
						...
						if ((
							cmd === 'insertunorderedlist' && listTag === 'OL'
							)
							|| (
				...
				// outdent
					...
					if (block && block.tagName == 'LI') {
						...
						$.each(elements, $.proxy(function (i, elem) {
							...
							if (left <= 0) {
								// linebreaks
								if (this.opts.linebreaks === true && typeof(
									$el.data('tagblock')
									) !== 'undefined') {
			...
			insertHtml: function (html, sync) {
				...
				if (!this.opts.linebreaks && $html.contents().length == 1 && $html.contents()[0].nodeType == 3
					&& (
					this.getRangeSelectedNodes().length > 2 || (
						!current || current.tagName == 'BODY' && !parent || parent.tagName == 'HTML'
						)
					)) {
					html = '<p>' + html + '</p>';
				}
			...
			insertLineBreak: function () {
				this.selectionSave();
				this.$editor.find('#selection-marker-1').before('<br>' + (
					this.browser('webkit') ? this.opts.invisibleSpace : ''
					));
				this.selectionRestore();
			},
			insertDoubleLineBreak: function () {
				this.selectionSave();
				this.$editor.find('#selection-marker-1').before('<br><br>' + (
					this.browser('webkit') ? this.opts.invisibleSpace : ''
					));
				this.selectionRestore();
			},
		...
		$.Redactor.fn.formatLinkify = function (protocol, convertLinks, convertImageLinks, convertVideoLinks) {
			...
			var childNodes = (
				this.$editor ? this.$editor.get(0) : this
			...
			while (i--) {
				...
				if (n.nodeType === 3) {
					...
					// link
					else if (convertLinks && html && (
						html.match(url1) || html.match(url2)
						)) {
						html = html.replace(/&/g, '&amp;')
							.replace(/</g, '&lt;')
							.replace(/>/g, '&gt;')
