...
(
	function () {
		CKEDITOR.on("dialogDefinition", function (a) {
			...
			a = a.data.definition;
			"link" == b ? (
				a.removeContents("target"), a.removeContents("upload"), a.removeContents("advanced"), b = a.getContents("info"), b.remove("emailSubject"), b.remove("emailBody")
				) : "image" == b && (
				a.removeContents("advanced"), b = a.getContents("Link"), b.remove("cmbTarget"), b = a.getContents("info"), b.remove("txtAlt"), b.remove("basic")
				)
		});
		...
		CKEDITOR.BBCodeParser.prototype = {parse: function (a) {
			for (var b, c, i = 0; b = this._.bbcPartsRegex.exec(a);) {
				...
				if ((
					...
					if (b = b[2]) {
						if ("list" == c && (
							isNaN(b) ? /^[a-z]+$/.test(b) ? b = "lower-alpha" : /^[A-Z]+$/.test(b) && (
								b = "upper-alpha"
								) : b = "decimal"
							), w[c]) {
							"size" == c && (
		...
		CKEDITOR.htmlParser.fragment.fromBBCode = function (a) {
			...
			function i(e, a) {
				...
				a.children.push(e);
				e.returnPoint && (
					d = e.returnPoint, delete e.returnPoint
					)
			}
		...
		var s = new (
			CKEDITOR.tools.createClass({$: function () {
				this._ = {output: [], rules: []};
				...
				a in j && (
					this.getRule(a, "breakBeforeClose") && this.lineBreak(1), "*" != a && this.write("[/", a, "]"), this.getRule(a, "breakAfterClose") && this.lineBreak(1)
					)
			}, text: function (a) {
				this.write(a)
			}, comment: function () {
			}, lineBreak: function () {
				!this._.hasLineBreak && this._.output.length && (
		...
		CKEDITOR.plugins.add("bbcode", {requires: "entities", beforeInit: function (a) {
			...
			i.addRules({elements: {blockquote: function (a) {
				...
				a.attributes.listType ? "decimal" != a.attributes.listType && (
					...
					if (f = c.href) {
						-1 !== f.indexOf("mailto:") ? (
							...
							(
								...
								d.type == CKEDITOR.NODE_TEXT && d.value == f
								) && (
								f = ""
								), d = "url"
							)
