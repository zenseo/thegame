...
CKEDITOR.dialog.add("link", function (o) {
	...
	var x = CKEDITOR.plugins.link, t = function () {
		...
		A = /subject=([^;?:@&=$,\/]*)/, B = /body=([^;?:@&=$,\/]*)/, C = /^#(.*)$/, D = /^((?:http|https|ftp|news):\/\/)?(.*)$/, E = /^(_(?:self|top|parent|blank))$/, F = /^javascript:void\(location\.href='mailto:'\+String\.fromCharCode\(([^)]+)\)(?:\+'(.*)')?\)$/, G = /^javascript:([^(]+)\(([^)]+)\)$/, H = /\s*window.open\(\s*this\.href\s*,\s*(?:'([^']*)'|null)\s*,\s*'([^']*)'\s*\)\s*;\s*return\s*false;*\s*/, I = /(?:^|,)([^=]+)=(\d+|yes|no)/gi, J = function (a, g) {
			var c = g && (
				g.data("cke-saved-href") || g.getAttribute("href")
				) || "", d, e, b =
			{};
			c.match(y) && (
				"encode" == p ? c = c.replace(F, function (a, c, b) {
					return"mailto:" + String.fromCharCode.apply(String, c.split(",")) + (
						b && b.replace(/\\'/g, "'")
						)
			...
			if (!b.type) {
				if (d = c.match(C)) {
					...
					f.address = d[1];
					e && (
						f.subject = decodeURIComponent(e[1])
						);
					c && (
						f.body = decodeURIComponent(c[1])
						)
				}
				else {
					c && (
			...
			if (g) {
				...
				b.adv = {};
				if (d) {
					d.match(E) ? b.target.type = b.target.name = d : (
	...
	p && "encode" != p && (
		r = q = void 0, p.replace(/^([^(]+)\(([^)]+)\)$/, function (a, b, c) {
			r = b;
			q = [];
	...
	return{title: b.title, minWidth: 350, minHeight: 230, contents: [
		...
		(
			c = x.getSelectedLink(a)
			) && c.hasAttribute("href") ? b.selectElement(c) : c = null;
