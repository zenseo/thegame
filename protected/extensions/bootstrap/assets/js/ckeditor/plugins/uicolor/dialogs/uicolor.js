...
CKEDITOR.dialog.add("uicolor", function (b) {
	function h(a) {
		/^#/.test(a) && (
			a = window.YAHOO.util.Color.hex2rgb(a.substr(1))
			);
		c.setValue(a, !0);
		c.refresh(e)
	}
	...
	function f(a, j) {
		(
			j || d._.contents.tab1.livePeview.getValue()
			) && b.setUiColor(a);
			...
			{id: "tab1", type: "vbox", children: [
				...
				{type: "hbox", children: [
					{id: "predefined", type: "select", "default": "",
						...
						var a = this.getValue();
						a ? (
							h(a), f(a), CKEDITOR.document.getById("predefinedPreview").setStyle("background", a)
