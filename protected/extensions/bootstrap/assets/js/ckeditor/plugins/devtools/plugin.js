...
(
	function () {
		function i(a, c, b, f) {
			var a = a.lang.devtools, j = '<a href="http://docs.cksource.com/ckeditor_api/symbols/CKEDITOR.dialog.definition.' + (
				b ? "text" == b.type ? "textInput" : b.type : "content"
				) + '.html" target="_blank">' + (
				b ? b.type : "content"
				) + "</a>", c = "<h2>" + a.title + "</h2><ul><li><strong>" + a.dialogName + "</strong> : " + c.getName() + "</li><li><strong>" + a.tabName + "</strong> : " + f + "</li>";
			b && (
				c += "<li><strong>" + a.elementId + "</strong> : " + b.id + "</li>"
				);
		...
		CKEDITOR.on("dialogDefinition", function (d) {
			...
			if (c._.showDialogDefinitionTooltips) {
				a || (
					a = CKEDITOR.dom.element.createFromHtml('<div id="cke_tooltip" tabindex="-1" style="position: absolute"></div>',
