...
CKEDITOR.dialog.add("scaytcheck", function (j) {
	...
	function x(b, a) {
		if (b) {
			...
			if (void 0 == e) {
				...
				for (var d = 0; d < e; d++) {
					b[d].checked = !1, b[d].value == a.toString() && (
						b[d].checked = !0
						)
				}
			}
	...
	var A = function () {
		...
		d = function (a, b) {
			...
			var d = f.createElement("div"), e = CKEDITOR.dom.element.createFromHtml('<input id="cke_option' + a + '" type="radio" ' + (
				k.sLang == a ? 'checked="checked"' : ""
				) + ' value="' + a + '" name="scayt_lang" />');
			e.on("click", function () {
				this.$.checked = true;
				k.chosed_lang = a
			});
		...
		if (1 == l[0]) {
			for (var g = w(), e = 0, d = g.length; e < d; e++) {
				...
				if (i && (
					g[e].checked = !1, 1 == b.options[h.split("_")[0]] && (
						g[e].checked = !0
						), s
					)) {
					i.on("click", function () {
		...
		1 == l[1] && (
			g = f.getById("cke_option" + b.sLang), x(g.$, b.sLang)
			);
		u && (
