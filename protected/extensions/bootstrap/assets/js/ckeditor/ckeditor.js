		...
		if (!window.CKEDITOR || !window.CKEDITOR.dom) {
			...
			CKEDITOR.event || (
				CKEDITOR.event = function () {
					...
					function () {
						...
						var b = function (a) {
							a = a.getPrivate && a.getPrivate() || a._ || (
								a._ = {}
								);
							return a.events || (
								a.events = {}
								...
								if (k) {
									...
									if (l.length) {
										for (var l = l.slice(0), n, o = 0; o < l.length; o++) {
											...
											n === false ? i = 1 : typeof n != "undefined" && (
												g = n
												);
											if (a || i) {
												break
			...
			CKEDITOR.env || (
				CKEDITOR.env = function () {
					...
					if (c.webkit) {
						a.indexOf("chrome") > -1 ? c.chrome = true : c.safari = true;
					}
					var h = 0;
					if (c.ie) {
					...
					c.opera && (
						h = parseFloat(b.version())
						);
					c.air && (
						h = parseFloat(a.match(/ adobeair\/(\d+)/)[1])
						);
					c.webkit && (
						h = parseFloat(a.match(/ applewebkit\/(\d+)/)[1])
						);
					c.version = h;
					c.isCompatible = c.iOS && h >= 534 || !c.mobile && (
						c.ie && h > 6 || c.gecko && h >= 10801 || c.opera && h >= 9.5 || c.air && h >=
							1 || c.webkit && h >= 522 || false
						);
					c.cssClass = "cke_browser_" + (
						c.ie ? "ie" : c.gecko ? "gecko" : c.opera ? "opera" : c.webkit ? "webkit" : "unknown"
						);
					if (c.quirks) {
					...
					if (c.ie) {
						c.cssClass = c.cssClass + (
							" cke_browser_ie" + (
								c.quirks || c.version < 7 ? "6" : c.version
								)
							);
						if (c.quirks) {
						...
						defer: function (a) {
							...
							a.replace(/&quot;/g, '"').replace(/\s*([^:;\s]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function (a, c, g) {
								if (b) {
									c = c.toLowerCase();
									c == "font-family" && (
										g = g.toLowerCase().replace(/["']/g, "").replace(/\s*,\s*/g, ",")
										);
			...
			CKEDITOR.tools.extend(CKEDITOR.dom.node.prototype, {appendTo: function (a, b) {
				...
				var c = this.$.cloneNode(a), h = function (c) {
					...
					if (c.nodeType == CKEDITOR.NODE_ELEMENT) {
						...
						if (a) {
							for (var c = c.childNodes, i = 0; i < c.length; i++) {
								h(c[i])
							}
						}
					}
				...
				do c = (
					b = b.previousSibling
					) && b.nodeType != 10 && new CKEDITOR.dom.node(b);
				while (c && a && !a(c));
				return c
			}, getNext: function (a) {
				var b = this.$, c;
				do c = (
					...
					new CKEDITOR.dom.node(b);
				while (c && a && !a(c));
				return c
			}, getParent: function (a) {
				var b = this.$.parentNode;
				return b && (
					b.nodeType == CKEDITOR.NODE_ELEMENT || a && b.nodeType == CKEDITOR.NODE_DOCUMENT_FRAGMENT
					) ? new CKEDITOR.dom.node(b) : null
			}, getParents: function (a) {
				var b = this, c = [];
				do c[a ? "push" : "unshift"](b);
				while (b = b.getParent());
				return c
				...
				if (b.compareDocumentPosition) {
					return b.compareDocumentPosition(c);
				}
				if (b == c) {
				...
				if (this.type == CKEDITOR.NODE_ELEMENT && a.type == CKEDITOR.NODE_ELEMENT) {
					...
					if ("sourceIndex"in b) {
						return b.sourceIndex < 0 || c.sourceIndex < 0 ? CKEDITOR.POSITION_DISCONNECTED :
				...
				for (var b = this.getAddress(), a = a.getAddress(), c = Math.min(b.length, a.length), h = 0; h <= c - 1; h++) {
					if (b[h] != a[h]) {
						if (h < c) {
							return b[h] < a[h] ? CKEDITOR.POSITION_PRECEDING : CKEDITOR.POSITION_FOLLOWING;
						}
						break
					}
				}
				...
				for (; c;) {
					if (c.nodeName &&
						(
							h = c.nodeName.toLowerCase(), typeof a == "string" ? h == a : h in a
							)) {
						return new CKEDITOR.dom.node(c);
					}
					c = c.parentNode
				...
				for (; c;) {
					if (c.nodeName && c.nodeName.toLowerCase() == a) {
						return true;
					}
					c = c.parentNode
				}
				...
				if (c) {
					if (a) {
						for (; a = b.firstChild;) {
							c.insertBefore(b.removeChild(a), b);
						}
					}
					c.removeChild(b)
				}
				...
				if (a && typeof a.$.isContentEditable != "undefined") {
					return!(
						a.$.isContentEditable || a.data("cke-editable")
						);
				}
				for (; a;) {
			...
			CKEDITOR.tools.extend(CKEDITOR.dom.window.prototype, {focus: function () {
				...
				return{width: (
					b ? a.documentElement.clientWidth : a.body.clientWidth
					) || 0, height: (
					b ? a.documentElement.clientHeight : a.body.clientHeight
					) ||
					0}
			...
			CKEDITOR.tools.extend(CKEDITOR.dom.document.prototype,
				{type: CKEDITOR.NODE_DOCUMENT, appendStyleSheet: function (a) {
					...
					for (var c = this.$.documentElement, h = 0; c && h < a.length; h++) {
						...
						if (b) {
							for (var i =
								...
								if (!(
									b === true && e.nodeType == 3 && e.previousSibling && e.previousSibling.nodeType == 3
									)) {
									i++;
									if (i == d) {
										c = e;
					...
					if ((
						!CKEDITOR.env.ie || document.documentMode > 8
						) && b) {
						a = b + ":" + a;
					}
			...
			CKEDITOR.dom.element.get = function (a) {
				return(
					a = typeof a == "string" ? document.getElementById(a) || document.getElementsByName(a)[0] : a
					) && (
			...
			CKEDITOR.dom.element.clearAllMarkers = function (a) {
				for (var b in a) {
					CKEDITOR.dom.element.clearMarkers(a,
						a[b], 1)
				}
			};
			...
			CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {type: CKEDITOR.NODE_ELEMENT, addClass: function (a) {
				...
				return CKEDITOR.env.ie && CKEDITOR.env.version < 9 ? function (a) {
					try {
						...
						h.$.innerHTML = a;
						for (h = h.getChildren(); h.count();) {
							this.append(h.getItem(0));
						}
						return a
					}
				} : a
				...
				var a = this.$.tabIndex;
				a === 0 && (
					!CKEDITOR.dtd.$tabIndex[this.getName()] && parseInt(this.getAttribute("tabindex"), 10) !== 0
					) && (
					a = -1
					);
				return a
				...
				getFirst: function (a) {
					...
					var a = (
						...
						) && this.getComputedStyle("visibility") != "hidden", b, c;
					if (a && (
						CKEDITOR.env.webkit || CKEDITOR.env.opera
						)) {
						b = this.getWindow();
						if (!b.equals(CKEDITOR.document.getWindow()) && (
							c = b.$.frameElement
							)) {
							a = (
					...
					for (var a = this.getChildren(), b = 0, c = a.count(); b < c; b++) {
						var h = a.getItem(b);
						if (!(
							h.type == CKEDITOR.NODE_ELEMENT && h.data("cke-bookmark")
							) && (
							h.type == CKEDITOR.NODE_ELEMENT && !h.isEmptyInlineRemoveable() || h.type == CKEDITOR.NODE_TEXT && CKEDITOR.tools.trim(h.getText())
							)) {
							return false
						}
					}
					...
					if (c != a) {
						...
						if (b) {
							for (; h = c.lastChild;) {
								a.insertBefore(c.removeChild(h),
									a.firstChild);
							}
						}
						else {
							for (; h = c.firstChild;) {
								a.appendChild(c.removeChild(h))
							}
						}
					}
					...
					function a(a, c, h) {
						if (c && c.type == CKEDITOR.NODE_ELEMENT) {
							...
							if (a.isIdentical(c)) {
								for (var i = h ? a.getLast() : a.getFirst(); d.length;) {
									d.shift().move(a, !h);
								}
								c.moveChildren(a, !h);
								c.remove();
					...
					return CKEDITOR.env.ie && (
						...
						function (a) {
							a == "class" ? a = "className" : a == "tabindex" ? a = "tabIndex" : a == "contenteditable" && (
								a = "contentEditable"
								);
							this.$.removeAttribute(a)
						} : a
				...
				setStyle: function (a, b) {
					...
					if (document.documentElement.getBoundingClientRect) {
						...
						if (k) {
							b = f.left + (
								!i && e.scrollLeft || d.$.scrollLeft
								);
							b = b - j;
							c = f.top + (
								!i && e.scrollTop || d.$.scrollTop
								);
							c = c - g
						}
					}
						...
						d = this;
						for (h = null; d && !(
							d.getName() == "body" || d.getName() == "html"
							);) {
							b = b + (
								d.$.offsetLeft - d.$.scrollLeft
								);
							c = c + (
					...
					if (a) {
						...
						if (!d.equals(h) && d.$.frameElement) {
							a = (
								new CKEDITOR.dom.element(d.$.frameElement)
						...
						do {
							(
								b.$.clientWidth && b.$.clientWidth <
									b.$.scrollWidth || b.$.clientHeight && b.$.clientHeight < b.$.scrollHeight
					...
					a instanceof CKEDITOR.dom.window && (
						a = k ? i.getBody() : i.getDocumentElement()
						);
					i = a.getWindow();
					d = g(this, i);
					...
					h = d.x + h + j(this, "right") - (
						q.x + o
						) || 0;
					d = d.y + l + j(this, "bottom") - (
						q.y + n
						) || 0;
					if (f < 0 || d > 0) {
						e(0, b === true ? f : b === false ? d : f < 0 ? f : d);
					}
					if (c && (
						i < 0 || h > 0
						)) {
			...
			(
				function () {
					function a(a) {
						for (var h = 0, d = 0, i = b[a].length; d < i; d++) {
							h = h + (
								parseInt(this.getComputedStyle(b[a][d]) || 0, 10) || 0
								);
						}
						return h
					}
			...
			(
				function () {
					function a(a, b) {
						...
						if (!a && !this._.guardLTR) {
							...
							this._.guardLTR = function (a, j) {
								return(
									...
									a.type != CKEDITOR.NODE_ELEMENT || !j || !a.equals(c.root)
									)
							}
						}
						...
						if (this.current) {
							j = this.current[p](false, m, i);
						}
						else {
							if (a) {
								j.type == CKEDITOR.NODE_ELEMENT && (
									j = d > 0 ? j.getChild(d - 1) : i(j, true) === false ? null : j.getPreviousSourceNode(true, m, i)
									);
							}
							else {
								j = k;
								if (j.type ==
									...
									j = i(k, true) === false ? null : k.getNextSourceNode(true, m, i)
								}
							}
							j && i(j) === false && (
								j = null
					...
					CKEDITOR.dom.walker.blockBoundary = function (a) {
						return function (b) {
							return!(
								b.type == CKEDITOR.NODE_ELEMENT && b.isBlockBoundary(a)
								)
						}
					};
					...
					CKEDITOR.dom.element.prototype.getBogus = function () {
						...
						while (i(a) ||
							d(a) || a.type == CKEDITOR.NODE_ELEMENT && a.getName()in CKEDITOR.dtd.$inline && !(
							a.getName()in CKEDITOR.dtd.$empty
							));
						return a && (
							!CKEDITOR.env.ie ? a.is && a.is("br") : a.getText && h.test(a.getText())
							) ? a : false
					}
				}
				)();
			...
			(
				function () {
					function a(a) {
						...
						return function (g) {
							...
							if (g.type == CKEDITOR.NODE_TEXT) {
								if (CKEDITOR.env.ie && e.test(g.getText()) && !b && (
									!a || !g.getNext()
									)) {
									b = true;
								}
								else {
									if (g.hasAscendant("pre") || CKEDITOR.tools.trim(g.getText()).length) {
										return false
									}
								}
							}
							else if (g.type == CKEDITOR.NODE_ELEMENT && !i[g.getName()]) {
								if (!CKEDITOR.env.ie && g.is("br") && !b && (
									!a || !g.getNext()
									)) {
									b = true;
								}
					...
					function b(a) {
						return function (b) {
							return!a && f(b) || (
								b.type == CKEDITOR.NODE_TEXT ? !CKEDITOR.tools.trim(b.getText()) || !!b.getParent().data("cke-bookmark") : b.getName()in CKEDITOR.dtd.$removeEmpty
								)
						}
					}
					...
					var h = function (a) {
						...
						for (var w = c, x, y, z, D = t; D < h.length; D++) {
							...
							for (x = x.getNext(); x;) {
								if (x.equals(d[D]) || x.equals(f)) {
									break;
								}
								z = x.getNext();
								if (b == 2) {
									w.append(x.clone(true));
								}
								else {
									x.remove();
									b == 1 && w.append(x)
						...
						for (c = t; c < d.length; c++) {
							...
							if (!h[c] || x.$.parentNode != h[c].$.parentNode) {
								for (x = x.getPrevious(); x;) {
									if (x.equals(h[c]) || x.equals(g)) {
										break;
									}
									z = x.getPrevious();
									if (b == 2) {
										w.$.insertBefore(x.$.cloneNode(true), w.$.firstChild);
									}
									else {
										x.remove();
						...
						if (b == 2) {
							...
							if (u && v && (
								g.$.parentNode != u.$.parentNode || f.$.parentNode != v.$.parentNode
								)) {
								b = v.getIndex();
					...
					CKEDITOR.dom.range.prototype = {clone: function () {
						...
						return{startNode: a ? e + (
							f ? "C" : "S"
							) : b, endNode: a ? e + "E" : c, serializable: a, collapsed: f}
						...
						if (a) {
							if (b.type == CKEDITOR.NODE_ELEMENT) {
								if ((
									h = b.getChild(e)
									) && h.type == CKEDITOR.NODE_TEXT && e > 0 && h.getPrevious().type ==
									CKEDITOR.NODE_TEXT) {
									b = h;
									e = 0
								}
							...
							if (!f) {
								if (c.type == CKEDITOR.NODE_ELEMENT) {
									if ((
										h = c.getChild(g)
										) && h.type == CKEDITOR.NODE_TEXT && g > 0 && h.getPrevious().type == CKEDITOR.NODE_TEXT) {
										c = h;
										g = 0
									}
									h && h.type == CKEDITOR.NODE_ELEMENT && (
						...
						a.getPosition(b) & CKEDITOR.POSITION_FOLLOWING && (
							a = b
							);
						...
						switch (a) {
							...
							case CKEDITOR.ENLARGE_ELEMENT:
								...
								for (; d || i;) {
									if (d && !i) {
										...
										if (c ? d.isBlockBoundary() : !g.contains(d)) {
											break;
										}
									...
									for (; i;) {
										...
										if (i.type == CKEDITOR.NODE_COMMENT) {
											...
											u && (
												t ? r ? f = d : d && this.setStartBefore(d) : t = true
												);
											if (i) {
												u = i.getPrevious();
												if (!d && !u) {
							...
							case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
								...
								d = d.lastBackward();
								z = z || g;
								this.setStartAt(z, !z.is("br") && (
									!d && this.checkStartOfBlock() || d && z.contains(d)
								...
								if (a == CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS) {
									...
									if ((
										d = d.previous()
										) && d.type == CKEDITOR.NODE_ELEMENT && d.is("br")) {
										break
									}
								}
								d = this.clone();
						...
						if (!this.collapsed) {
							...
							e.evaluator = function (b) {
								return b.type == (
									a == CKEDITOR.SHRINK_ELEMENT ? CKEDITOR.NODE_ELEMENT : CKEDITOR.NODE_TEXT
									)
							};
							var u;
							e.guard = function (b, e) {
								...
								if (a == CKEDITOR.SHRINK_ELEMENT && b.type == CKEDITOR.NODE_TEXT || e && b.equals(u) || c === false && b.type == CKEDITOR.NODE_ELEMENT && b.isBlockBoundary()) {
									return false;
								}
							...
							if (i) {
								(
									g = e[a == CKEDITOR.SHRINK_ELEMENT ? "lastForward" :
										"next"]()
						...
						moveToPosition: function (a, b) {
							...
							if (a != "br") {
								if (!e) {
									...
									g = (
										new CKEDITOR.dom.elementPath(this.endContainer, this.root)
										).block
								}
								g || (
									g = this.fixBlock(false, a)
									)
							}
							...
							function a(e) {
								return function (a) {
									return b(a) || (
										...
										) || e.is("table") && a.is("caption") ? false : true
								}
							}
							...
							if (c && b.type == CKEDITOR.NODE_TEXT && CKEDITOR.tools.ltrim(b.substring(0, c)).length) {
								return false;
							}
							this.trim();
							...
							if (b.type == CKEDITOR.NODE_TEXT && CKEDITOR.tools.rtrim(b.substring(c)).length) {
								return false;
							}
							this.trim();
							...
							function a(b, c) {
								for (; b;) {
									if (b.type == CKEDITOR.NODE_ELEMENT) {
										...
										if (b.is("html") || b.getAttribute("contentEditable") == "true" && (
							...
							for (var g = 0; a;) {
								...
								if (a.type == CKEDITOR.NODE_ELEMENT) {
									if (a.isEditable()) {
										...
										b && (
											a.is("br") && this.checkEndOfBlock()
											) && this.moveToPosition(a, CKEDITOR.POSITION_BEFORE_START);
									}
								}
								var f = a, h = g, d = void 0;
								f.type == CKEDITOR.NODE_ELEMENT && f.isEditable(false) && (
									d = f[b ? "getLast" : "getFirst"](c)
									);
								!h && !d && (
									d = f[b ? "getPrevious" : "getNext"](c)
									);
								a = d
							}
							return!!g
				...
				function () {
					...
					a.prototype = {getNextParagraph: function (a) {
						...
						for (this._.nextNode = null; n;) {
							...
							if (p) {
								n.type == CKEDITOR.NODE_TEXT && c.test(n.getText()) && (
									p = 0
									);
							}
							else {
						...
						if (!e) {
							...
							o = {div: 1, th: 1, td: 1};
							e = e.block;
							if (!e && n && !this.enforceRealBlocks && o[n.getName()] && g.checkStartOfBlock() && g.checkEndOfBlock() && !n.equals(g.root)) {
								e = n;
							}
								...
								this._.nextNode = e.equals(d) ? null : b(g.getBoundaryNodes().endNode, 1, d)
							}
						}
						if (q) {
							(
								...
								g.getName() == "br" ? g.remove() : g.getLast() && g.getLast().$.nodeName.toLowerCase() == "br" && g.getLast().remove()
								);
						}
						if (l) {
							(
								...
								g.type == CKEDITOR.NODE_ELEMENT && g.getName() == "br" && (
								CKEDITOR.env.ie || g.getPrevious(h) || g.getNext(h)
								) && g.remove();
						}
						if (!this._.nextNode) {
							this._.nextNode = j || e.equals(d) || !d ? null : b(e, 1, d);
						}
						return e
					}};
			...
			(
				function () {
					var a, b = function (b) {
						...
						if (!a) {
							i && (
								a = f.execCommand(i, {from: "keystrokeHandler"}) !== false
								);
							a || (
								a = !!this.blockedKeystrokes[c]
								)
						}
			...
			{add: function (a, b) {
				...
				for (var a = a.split(","), h = 0; h < a.length; h++) {
					this.externals[a[h]] = {dir: b, file: c}
				}
			}, load: function (a, b, c) {
						...
						a.call(this, c, function (a) {
							...
							for (e in a) {
								...
								if (!b[e]) {
									if (q.icons) {
										for (var n =
											q.icons.split(","), o = 0; o < n.length; o++) {
											CKEDITOR.skin.addIcon(n[o], q.path + "icons/" + n[o] + ".png");
										}
									}
									b[e] = 1
								}
			...
			CKEDITOR.plugins.setLang = function (a, b, c) {
				var h = this.get(a), a = h.langEntries || (
					h.langEntries = {}
					), h = h.lang || (
					h.lang = []
					);
				...
				function () {
					function a(a, e, g) {
						...
						if (e !== void 0) {
							...
							if (g == CKEDITOR.ELEMENT_MODE_INLINE && !e.is(CKEDITOR.dtd.$editable) || g == CKEDITOR.ELEMENT_MODE_REPLACE && e.is(CKEDITOR.dtd.$nonBodyContent)) {
					...
					function g(a) {
						...
						if (e) {
							var f = RegExp("(?:^|,)(?:" + e.replace(/\s*,\s*/g, "|") + ")(?=,|$)", "g"), c = c.replace(f, ""), c = c + (
								"," + e
								);
						}
						if (g) {
						...
						CKEDITOR.plugins.load(c.split(","), function (c) {
							...
							for (var j in c) {
								...
								if (h && !a.lang[j]) {
									...
									if (!d.langEntries || !d.langEntries[i]) {
										f.push(CKEDITOR.getUrl(d.path + "lang/" + i + ".js"));
									}
									else {
							...
							CKEDITOR.scriptLoader.load(f, function () {
								for (var c = ["beforeInit", "init", "afterInit"], f = 0; f < c.length; f++) {
									for (var j = 0; j < e.length; j++) {
										var d = e[j];
										f === 0 && (
											g[j] && d.lang && d.langEntries
											) && (
											a.lang[d.name] = d.langEntries[g[j]]
											);
										if (d[c[f]]) {
											d[c[f]](a)
										}
								...
								for (j = 0; j < a.config.blockedKeystrokes.length; j++) {
									a.keystrokeHandler.blockedKeystrokes[a.config.blockedKeystrokes[j]] =
										1;
								}
								a.fireOnce("loaded");
					...
					CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {addCommand: function (a, b) {
						...
						destroy: function (a) {
							...
							return(
								a = a || this.getSelection().getStartElement()
							...
							if (typeof b != "string") {
								b = (
									b = this.element
									) && this.elementMode == CKEDITOR.ELEMENT_MODE_REPLACE ? b.is("textarea") ? b.getValue() : b.getHtml() : "";
							}
							b = {dataValue: b};
			...
			(
				function () {
					...
					CKEDITOR.htmlParser.prototype = {onTagOpen: function () {
						...
						if (c.length >
							i) {
							this.onText(c.substring(i, c.length))
						}
					}}
				}
				)();
				...
				function () {
					...
					CKEDITOR.htmlParser.fragment.fromHtml = function (i, f, e) {
							...
							if (b.previous === void 0) {
								...
								if (e && c.name == "body") {
									...
									if ((
										...
										j in CKEDITOR.dtd.$body || j == "body" || b.isOrphan
										)) {
										m = c;
										q.onTagOpen(e, {});
										b.returnPoint = c = m
								...
								if (b._.isBlockLike &&
									...
									if (d && d.type == CKEDITOR.NODE_TEXT) {
										(
											h = CKEDITOR.tools.rtrim(d.value)
											) ? d.value = h : b.children.length = j - 1
									}
								}
								(
									!a(b) || b.children.length
									) && c.add(b);
								b.name == "pre" && (
									s = false
									);
									...
									for (; ;) {
										...
										if (!f.isUnknown && !m.isUnknown && !l[e]) {
											if (m.isOptionalClose) {
												...
												(
													i = i[i.length - 1]
													) && i.name == "li" || k(i = new CKEDITOR.htmlParser.element("li"), m);
												!f.returnPoint && (
													f.returnPoint = m
													);
												m = i
											}
											else if (e in CKEDITOR.dtd.$listItem && i != e) {
												q.onTagOpen(e == "li" ? "ul" : "dl", {}, 0, 1);
											}
											else if (i in b && i != e) {
												!f.returnPoint && (
													f.returnPoint = m
						...
						q.onText = function (a) {
							...
							var c = m.name, f = c ? CKEDITOR.dtd[c] || (
								m._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span
								) : d;
							if (!p && !f["#"] && c in b) {
								...
								if (e && (
									!m.type || m.name == "body"
									) && CKEDITOR.tools.trim(a)) {
									this.onTagOpen(e, {}, 0, 1);
								}
								!s && !p && (
									a = a.replace(/[\t\r\n ]{2,}|[\t\r\n]/g, " ")
									);
					...
					CKEDITOR.htmlParser.fragment.prototype = {add: function (a, b) {
						...
						this.filterChildren = function () {
							...
							a = a.getHtml();
							this.children = (
								new CKEDITOR.htmlParser.fragment.fromHtml(a)
								).children;
							c = 1
						};
			...
			(
				function () {
					...
					function b(a) {
						for (var b = a.children.length - 1, c = a.children[b]; c && (
							c.type == CKEDITOR.NODE_TEXT && !CKEDITOR.tools.trim(c.value) || c.type == CKEDITOR.NODE_ELEMENT &&
								c.attributes["data-cke-bookmark"]
							);) {
							c = a.children[--b];
						}
						return b
					...
					function c(a, c) {
						...
						j && (
							(
								c || !CKEDITOR.env.ie
								) && j.type == CKEDITOR.NODE_ELEMENT && j.name == "br" ? e.splice(g, 1) : j.type == CKEDITOR.NODE_TEXT && n.test(j.value) && e.splice(g, 1)
							)
					}
					...
					function h(a, e) {
						return function (g) {
							...
							j = !a;
							if (!j && (
								!e || typeof e == "function" && e(g) === false
								)) {
								j = false;
							}
							else if (j && CKEDITOR.env.ie && (
								document.documentMode > 7 || g.name in CKEDITOR.dtd.tr || g.name in CKEDITOR.dtd.$listItem
								)) {
								j = false;
							}
							else {
								...
								j =
									g.children[j];
								j = !j || j && (
									j.type == CKEDITOR.NODE_ELEMENT && j.name == "br" || g.name == "form" && j.name == "input"
									)
							}
							j && (
								a || CKEDITOR.env.ie ? g.add(new CKEDITOR.htmlParser.text(" ")) : g.add(new CKEDITOR.htmlParser.element("br", {}))
								)
						}
					}
					...
					function l(a, b) {
						for (var c = [], e = b.config.protectedSource, g = b._.dataStore || (
							b._.dataStore = {id: 1}
							), j = /<\!--\{cke_temp(comment)?\}(\d*?)--\>/g, e = [/<script[\s\S]*?<\/script>/gi,
							/<noscript[\s\S]*?<\/noscript>/gi].concat(e), a = a.replace(/<\!--[\s\S]*?--\>/g, function (a) {
							return"<\!--{cke_tempcomment}" + (
								c.push(a) - 1
								) + "--\>"
						}), f = 0; f < e.length; f++) {
							a = a.replace(e[f], function (a) {
								a = a.replace(j, function (a, b, e) {
									return c[e]
								});
								return/cke_temp(comment)?/.test(a) ? a : "<\!--{cke_temp}" + (
									c.push(a) - 1
									) + "--\>"
							});
						}
					...
					CKEDITOR.htmlDataProcessor = function (a) {
						...
						b.root = h(true, false);
						for (r in s) {
							b.elements[r] = h(true, a.config.fillEmptyBlocks);
						}
						c.addRules(b)
					};
			...
			CKEDITOR.htmlParser.element = function (a, b) {
				...
				c = !(
					!CKEDITOR.dtd.$nonBodyContent[c] && !CKEDITOR.dtd.$block[c] && !CKEDITOR.dtd.$listItem[c] && !CKEDITOR.dtd.$tableContent[c] && !(
						CKEDITOR.dtd.$nonEditable[c] || c == "br"
						)
					);
			...
			CKEDITOR.htmlParser.cssStyle = function (a) {
				...
				(
					...
					).replace(/&quot;/g, '"').replace(/\s*([^ :;]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function (a, h, d) {
						h == "font-family" && (
							d = d.replace(/["']/g,
								"")
							);
					...
					CKEDITOR.htmlParser.element.prototype = {type: CKEDITOR.NODE_ELEMENT, add: CKEDITOR.htmlParser.fragment.prototype.add, clone: function () {
						...
						d.filterChildren = function () {
							if (!j) {
								...
								d.children = (
									new CKEDITOR.htmlParser.fragment.fromHtml(a.getHtml(), d.clone(), 0)
									).children;
								j = 1
							}
						};
							...
							for (; ;) {
								...
								if (!i) {
									for (var i = 0, k = this.children.length; i < k; i++) {
										this.children[i].parent = d.parent;
									}
						...
						for (var k = [], q = 0; q < 2; q++) {
							for (f in h) {
								...
								if (q == 1) {
									...
									e && (
										(
											g = c.onAttribute(d, e, g)
											) === false ? delete h[e] : h[e] = g
										)
								}
							}
						}
						...
						for (var a = document.getElementsByTagName("textarea"), b = 0; b < a.length; b++) {
							...
							if (g.name || g.id) {
								if (typeof arguments[0] == "string") {
									if (!RegExp("(?:^|\\s)" + arguments[0] + "(?:$|\\s)").test(g.className)) {
										continue
									}
								}
			...
			(
				function () {
					...
					CKEDITOR.editable = CKEDITOR.tools.createClass({base: CKEDITOR.dom.element, $: function (a, b) {
						...
						CKEDITOR.env.safari && !this.isInline() && (
						...
						if (!b) {
							var h = /\n{2}/g;
							if (h.test(a)) {
								var i = "<" + c + ">", n = "</" + c + ">", a = i + a.replace(h, function () {
									return n +
										i
								}) + n
							}
						...
						for (var c = this.editor, j = c.config.enterMode, f = c.getSelection(), d = f.getRanges(), h = a.getName(), i = CKEDITOR.dtd.$block[h], o, m, p, s = d.length - 1; s >= 0; s--) {
							...
							if (!o.checkReadOnly()) {
								...
								if (i) {
									for (; (
										r = o.getCommonAncestor(0, 1)
										) && (
										t = CKEDITOR.dtd[r.getName()]
										) && (
										!t || !t[h]
										);) {
										if (r.getName()in CKEDITOR.dtd.span) {
											...
											o.splitBlock(j == CKEDITOR.ENTER_DIV ? "div" : "p", c.editable());
										}
									}
								}
								o.insertNode(m);
						...
						if (p) {
							...
							if (i) {
								if ((
									a = p.getNext(b)
									) && a.type == CKEDITOR.NODE_ELEMENT && a.is(CKEDITOR.dtd.$block)) {
									a.getDtd()["#"] ? o.moveToElementEditStart(a) :
								...
								if (c in{8: 1, 46: 1}) {
									...
									if (j) {
										...
										if ((
											...
											) && g.checkBoundaryOfElement(k, c ? CKEDITOR.START : CKEDITOR.END) && (
											i = k[c ? "getPrevious" : "getNext"](d)
											)) {
											b.fire("saveSnapshot");
											g["moveToElementEdit" +
												(
						...
						a !==
							null && (
							a ? this.setAttribute("dir", a) : this.removeAttribute("dir")
							);
					...
					CKEDITOR.editor.prototype.editable = function (a) {
						...
						if (arguments.length) {
							b = this._.editable = a ? a instanceof CKEDITOR.editable ? a : new CKEDITOR.editable(this, a) : (
								b && b.detach(), null
								);
						}
						return b
								...
								if (!c.readOnly) {
									...
									if (e && !e.isLocked) {
										...
										if (CKEDITOR.env.gecko) {
											...
											if (r && r.isBlockBoundary() && (
												!t || !(
													t.type == CKEDITOR.NODE_ELEMENT && t.isBlockBoundary()
													)
												) && !r.is("pre") && !r.getBogus()) {
												r.appendBogus();
												s = 1
										...
										if (f.config.autoParagraph !== false && p != CKEDITOR.ENTER_BR && m.collapsed && i.equals(o) && !n.block) {
											...
											if (f.getOuterHtml().match(h)) {
												if ((
													...
													!i.isBlockBoundary() || !CKEDITOR.dtd.$empty[i.getName()]
													)) {
					...
					var f =
						function () {
							function a(b, c) {
								...
								return e && (
									f.block && f.block.is(o.$listItem) || f.lastElement.is(o.$list)
									) ? g() : 0
							}
							...
							function h(a) {
								return a && f(a) && (
									a.is(o.$removeEmpty) || a.is("a") && !a.isBlockBoundary()
									)
							}
							...
							function i(a, b, c, g) {
								...
								if ((
									d = (
										new CKEDITOR.dom.walker(e)
										).next()
									) && f(d) && m[d.getName()] && (
									h = d.getPrevious()
									) && f(h) && !h.getParent().equals(a.startContainer) &&
							...
							return function (m, t, u) {
								...
								if (!w.checkReadOnly()) {
									var x = (
										...
										).blockLimit || w.root, t = {type: t, editable: m, editor: v, range: w, blockLimit: x, mergeCandidates: [], zombies: []};
									if (!a(t, u)) {
										...
										if (!v.collapsed) {
											...
											y && (
												!y.equals(F) && !y.contains(F) && v.checkEndOfBlock()
												) && t.zombies.push(y);
											v.deleteContents()
										}
										for (; (
											B = f(v.startContainer) && v.startContainer.getChild(v.startOffset - 1)
											) && f(B) && B.isBlockBoundary() && D.contains(B);) {
											v.moveToPosition(B,
										...
										z = v.createBookmark();
										(
											y = z.startNode.getPrevious(b)
											) && f(y) && h(y) && x.push(y);
										(
											y = z.startNode.getNext(b)
											) && f(y) && h(y) && x.push(y);
										for (y = z.startNode; (
											y = y.getParent()
											) && h(y);) {
										...
										if (u) {
											...
											var O;
											J = D.getPosition(A);
											J = !!D.getCommonAncestor(A) && J != CKEDITOR.POSITION_IDENTICAL && !(
												J & CKEDITOR.POSITION_CONTAINS + CKEDITOR.POSITION_IS_CONTAINED
												);
											...
											for (var N = H.getChildren(), R = N.count(), S = -1, U = -1, V = 0; C < R; ++C) {
												...
												if (f(H)) {
													M = H.getName();
													Q = !!A[M];
													if (M == "br" && H.data("cke-eol") && (
														!C || C == R - 1
														)) {
														V = (
															G = C ? F[C - 1].node : N.getItem(C +
																1)
															) && (
															!f(G) || !G.is("br")
															);
													...
													F.push({isElement: 0, node: H, allowed: 1})
												}
											}
											if (S > -1) {
												F[S].firstNotAllowed = 1;
											}
											if (U > -1) {
												F[U].lastNotAllowed = 1;
											}
											for (n(F, E); B < F.length; B++) {
												...
												if (A = G.isLineBreak) {
													...
													if (G.hasBlockSibling) {
														...
														if (!H || !H.is({div: 1, p: 1})) {
															...
															M = H.getPosition(C);
															if (M == CKEDITOR.POSITION_IDENTICAL || M == CKEDITOR.POSITION_CONTAINS) {
																A = 0;
															}
															else {
																C = A.splitElement(H);
												...
												if (A) {
													...
													A = E.startPath();
													if (!G.isBlock && (
														I = t.editor.config.enterMode != CKEDITOR.ENTER_BR && t.editor.config.autoParagraph !== false ? t.editor.config.enterMode == CKEDITOR.ENTER_DIV ? "div" : "p" : false
														) && !A.block && A.blockLimit && A.blockLimit.equals(E.root)) {
														I = u.createElement(I);
													...
													if (G.lastNotAllowed && B < F.length - 1) {
														(
															P = J ? D : P
															) && E.setEndAt(P, CKEDITOR.POSITION_AFTER_START);
														x = 0
													}
													E.collapse()
												}
										...
										if (P) {
											for (; L =
												P.pop();) {
												L.append(CKEDITOR.env.ie ? K.document.createText(" ") : K.document.createElement("br"));
											}
										}
										for (; L = t.mergeCandidates.pop();) {
											L.mergeSiblings();
										}
										K.moveToBookmark(O);
										if (!t.dontMoveCaret) {
											for (L = f(K.startContainer) && K.startContainer.getChild(K.startOffset - 1); L && f(L) && !L.is(o.$empty);) {
												if (L.isBlockBoundary()) {
													K.moveToPosition(L, CKEDITOR.POSITION_BEFORE_END);
												}
												else {
				...
				function () {
							...
							e.on("contentDom", function () {
								...
								if (CKEDITOR.env.ie && !j) {
									...
									g.attachListener(g, "mousedown", function (a) {
										a.data.$.button == 2 && e.document.$.selection.type == "None" && (
											k = e.window.getScrollPosition()
											)
									});
					...
					CKEDITOR.on("instanceReady", function (a) {
						...
						if (CKEDITOR.env.webkit) {
							b.on("selectionChange", function () {
								var a = d(c);
								a && (
									a.getCustomData("ready") ? i(c) : a.setCustomData("ready", 1)
									)
							}, null, null, -1);
					...
					CKEDITOR.dom.selection.prototype = {getNative: function () {
						...
						if (!c || !c.rangeCount) {
							...
							if (e == c.endContainer && e.nodeType == 1 && c.endOffset - c.startOffset == 1 && g[e.childNodes[c.startOffset].nodeName.toLowerCase()]) {
								b = CKEDITOR.SELECTION_ELEMENT
							}
						}
						return a.type = b
								...
								for (var g = c.ranges, e = 0; e < g.length; e++) {
									...
									if (!f.collapsed) {
										...
										b && b.type == CKEDITOR.NODE_TEXT && (
											h >= b.getLength() ? r.setStartAfter(b) : r.setStartBefore(b)
											);
										...
										b.evaluator = function (a) {
											if (a.type == CKEDITOR.NODE_ELEMENT && a.isReadOnly()) {
												...
												if (!(
													a.getPosition(r.endContainer) & CKEDITOR.POSITION_CONTAINS
													)) {
													b.setStartAfter(a);
						...
						var b = this, c = CKEDITOR.tools.tryThese(function () {
								...
								d = function (a, b) {
									return function (c, g) {
										if (c.type == CKEDITOR.NODE_TEXT && (
											!CKEDITOR.tools.trim(c.getText()) || c.getParent().data("cke-bookmark")
											)) {
											return true;
										}
										var e;
										if (c.type == CKEDITOR.NODE_ELEMENT) {
											e = c.getName();
											if (e == "br" && b && c.equals(c.getParent().getBogus()) || g && e in h || e in CKEDITOR.dtd.$removeEmpty) {
												return true
											}
										}
										a.halted = 1;
							...
							for (var a = b.getRanges()[0], c, e, f = 2; f && (
								!(
									c = a.getEnclosedNode()
									) || !(
									c.type == CKEDITOR.NODE_ELEMENT && g[c.getName()] && (
										e = c
										)
									)
								); f--) {
						...
						if (a.length) {
							if (this.isLocked) {
								...
								if (CKEDITOR.env.ie) {
									...
									(
										b.startContainer.type == CKEDITOR.NODE_ELEMENT && b.startContainer.getName()in f || b.endContainer.type == CKEDITOR.NODE_ELEMENT && b.endContainer.getName()in f
									...
									if (t) {
										...
										d = !(
											d && d.getText && d.getText().match(e)
											) && (
											m || !f.hasPrevious() || f.getPrevious().is && f.getPrevious().is("br")
											);
									...
									for (e = 0; e < a.length; e++) {
										...
										b.collapsed && (
											(
												...
												) && d.type == CKEDITOR.NODE_ELEMENT && !d.getChildCount()
											) && d.appendText("");
										if (b.collapsed && CKEDITOR.env.webkit && h(b)) {
											...
											if ((
												d = m.getNext()
												) && !m.getPrevious() && d.type == CKEDITOR.NODE_ELEMENT && d.getName() == "br") {
												i(this.root);
				...
				function () {
					...
					function b(b) {
						...
						if (b.collapsed) {
							...
							h == void 0 && (
								h = c.getCustomData("cke_includeReadonly")
								);
							...
							if (!d) {
								...
								d && (
									n = d.getNextSourceNode(true)
									);
								p && (
									q = p
									)
							}
							for (n.getPosition(q) == CKEDITOR.POSITION_FOLLOWING && (
								n = 0
								); n;) {
								d = false;
								if (n.equals(q)) {
									...
									if (!s || j[s] && !r && (
										...
										n.getPosition(q) | CKEDITOR.POSITION_PRECEDING | CKEDITOR.POSITION_IDENTICAL | CKEDITOR.POSITION_IS_CONTAINED
										) ==
										...
										if (x && (
											(
												x.getDtd() || CKEDITOR.dtd.span
												)[g] || f
											) && (
											!e.parentRule || e.parentRule(x)
											)) {
											if (!o && (
											...
											if (u == CKEDITOR.NODE_TEXT ||
												...
												for (var u = n, t; (
													...
													t = u.getParent(), j[t.getName()]
													) && (
													t.getPosition(k) | CKEDITOR.POSITION_FOLLOWING | CKEDITOR.POSITION_IDENTICAL | CKEDITOR.POSITION_IS_CONTAINED
													) == CKEDITOR.POSITION_FOLLOWING + CKEDITOR.POSITION_IDENTICAL + CKEDITOR.POSITION_IS_CONTAINED && (
													!e.childRule || e.childRule(t)
													);) {
													u = t;
												}
												o.setEndAfter(u)
										...
										if (r.getName() == g) {
											for (w in e.attributes) {
												if (!v[w] && (
													...
													d.getAttribute(w) == D ? s[w] = 1 : v[w] = 1;
												}
											}
											for (y in e.styles) {
												if (!x[y] && (
					...
					function c(a) {
						...
						if (a.collapsed) {
							for (var g = new CKEDITOR.dom.elementPath(c.getParent(), a.root), e, f = 0, d; f < g.elements.length && (
								d = g.elements[f]
								); f++) {
								if (d == g.block || d == g.blockLimit) {
									break;
								}
								if (this.checkElementRemovable(d)) {
									var h;
									if (a.collapsed && (
										a.checkBoundaryOfElement(d, CKEDITOR.END) || (
											h = a.checkBoundaryOfElement(d, CKEDITOR.START)
											)
										)) {
										e =
											d;
							...
							var j = b.endNode, i = this, g = function () {
								for (var a = new CKEDITOR.dom.elementPath(c.getParent()), b = new CKEDITOR.dom.elementPath(j.getParent()), g = null, e = null, f = 0; f < a.elements.length; f++) {
									...
									i.checkElementRemovable(d) && (
										g = d
										)
								}
								for (f = 0; f < b.elements.length; f++) {
									d = b.elements[f];
									if (d == b.block || d == b.blockLimit) {
										break;
									}
									i.checkElementRemovable(d) && (
										e = d
					...
					function h(a) {
						...
						(
							a = (
								new CKEDITOR.dom.elementPath(b, a.root)
								).contains(this.element, 1)
							) && !a.isReadOnly() && p(a, this)
					}
					...
					function e(a, b) {
						...
						if (e && !f) {
							...
							d ? b = k(c ? [a.getHtml()] : g(a), b) : a.moveChildren(b);
						}
						b.replace(a);
						if (e) {
							var c = b, i;
					...
					function q(a) {
						...
						for (var d in b) {
							if (!(
								this._.definition.fullMatch && a.getStyle(d) != t(d, b[d], true)
								)) {
								e = e || !!a.getStyle(d);
								a.removeStyle(d)
							}
						...
						n(a, g, v[a.getName()]);
						e && (
							this._.definition.alwaysRemoveElement ? o(a, 1) : !CKEDITOR.dtd.$block[a.getName()] || this._.enterMode == CKEDITOR.ENTER_BR && !a.hasAttributes() ? o(a) : a.renameNode(this._.enterMode == CKEDITOR.ENTER_P ? "p" : "div")
							)
					}

					function l(a) {
						for (var b =
							r(this), c = a.getElementsByTag(this.element), g = c.count(); --g >= 0;) {
							q.call(this, c.getItem(g));
						}
						for (var e in b) {
							if (e != this.element) {
						...
						if (b = b && b.attributes) {
							for (var g = 0; g < b.length; g++) {
								...
								if (f = a.getAttribute(e)) {
									var d = b[g][1];
									(
										d === null || d.test && d.test(f) || typeof d == "string" && f == d
										) && a.removeAttribute(e)
								}
							}
						}
					...
					function o(a, b) {
						if (!a.hasAttributes() || b) {
							if (CKEDITOR.dtd.$block[a.getName()]) {
								...
								g && (
									g.type == CKEDITOR.NODE_TEXT || !g.isBlockBoundary({br: 1})
									) && a.append("br");
								a.remove(true)
							}
							else {
								...
								if (c) {
									...
									g && (
										!c.equals(g) && g.type == CKEDITOR.NODE_ELEMENT
										) && g.mergeSiblings()
								}
							}
						}
					...
					function s(a, b) {
						for (var c in a) {
							a[c] = a[c].replace(y, function (a, c) {
								return b[c]
							})
						}
					}
					...
					function u(a, b) {
						for (var c = a.document, g = a.getRanges(), e = b ? this.removeFromRange : this.applyToRange, f, d = g.createIterator(); f = d.getNextRange();) {
							e.call(this,
								f);
						}
						a.selectRanges(g);
					...
					CKEDITOR.style = function (a, b) {
						...
						this.type = a.type || (
							v[c] ? CKEDITOR.STYLE_BLOCK : w[c] ? CKEDITOR.STYLE_OBJECT : CKEDITOR.STYLE_INLINE
							);
					...
					CKEDITOR.style.prototype = {apply: function (a) {
						...
						return(
							this.applyToRange = this.type == CKEDITOR.STYLE_INLINE ? b : this.type == CKEDITOR.STYLE_BLOCK ? i : this.type == CKEDITOR.STYLE_OBJECT ? h : null
							).call(this, a)
					}, removeFromRange: function (a) {
						return(
							this.removeFromRange = this.type == CKEDITOR.STYLE_INLINE ? c : this.type == CKEDITOR.STYLE_BLOCK ? f : this.type == CKEDITOR.STYLE_OBJECT ? d : null
							).call(this,
								a)
								...
								for (var b = a.elements, c = 0, g; c < b.length; c++) {
									...
									if (!(
										this.type == CKEDITOR.STYLE_INLINE && (
											g == a.block || g == a.blockLimit
											)
										)) {
										if (this.type == CKEDITOR.STYLE_OBJECT) {
											var e = g.getName();
											if (!(
												typeof this.element == "string" ? e == this.element : e in this.element
												)) {
												continue
											}
										}
									...
									if (h != "_length") {
										...
										if (h == "style") {
											a:{
												g = c[h];
												typeof g == "string" && (
													g = CKEDITOR.tools.parseCssText(g)
													);
												typeof e == "string" && (
												...
												for (d in g) {
													if (!(
														d in e && (
															e[d] == g[d] || g[d] == "inherit" || e[d] == "inherit"
															)
														)) {
														g = false;
														break a
						...
						if (c) {
							...
							for (var e = 0; e < c.length; e++) {
								...
								if (g = a.getAttribute(g)) {
									var f = c[e][1];
									if (f === null || typeof f == "string" && g == f || f.test(g)) {
										return true
									}
								}
							}
			...
			(
				function () {
					...
					for (b in CKEDITOR.dtd.$blockLimit) {
						b in CKEDITOR.dtd.$list || (
							a[b] = 1
							);
					}
					var c = {};
					for (b in CKEDITOR.dtd.$block) {
						b in CKEDITOR.dtd.$blockLimit || b in CKEDITOR.dtd.$empty || (
							c[b] = 1
							);
					}
			...
			CKEDITOR.dom.elementPath.prototype = {compare: function (a) {
				...
				} : typeof a == "function" ? h = a : typeof a == "object" && (
					h = function (b) {
			...
			(
				function () {
					...
					CKEDITOR.dom.rangeList = function (a) {
						...
						a ? a instanceof CKEDITOR.dom.range && (
							a = [a]
							) : a =
							[];
					...
					var b = {createIterator: function () {
							...
							if (e && a.length > 1) {
								if (!i) {
									for (var g = a.length - 1; g >= 0; g--) {
										d.unshift(a[g].createBookmark(true));
									}
								}
								if (f) {
									for (var j = 0; a[i + j + 1];) {
										for (var k = e.document, f = 0, g = k.getById(d[j].endNode), k = k.getById(d[j + 1].startNode); ;) {
											...
											else if (b(g) || g.type == CKEDITOR.NODE_ELEMENT && g.isBlockBoundary()) {
												continue;
											}
											break
										}
						...
						for (var b = [], d = 0; d < this.length; d++) {
							b.push(this[d].createBookmark2(a));
						}
						return b
			...
			(
				function () {
					...
					function b(b) {
						...
						if (c) {
							for (var c = c.split(",").sort(function (a, b) {
								...
								f = c[e];
								if (g.ie && (
									f.replace(/^ie/, "") == g.version || g.quirks && f == "iequirks"
									)) {
									f = "ie";
								}
								if (g[f]) {
					...
					CKEDITOR.skin = {path: a, loadPart: function (b, g) {
						...
						a = a.toLowerCase();
						this.icons[a] || (
							this.icons[a] = {path: b, offset: c || 0}
							)
			...
			CKEDITOR.plugins.add("dialogui", {onLoad: function () {
				...
				CKEDITOR.tools.extend(CKEDITOR.ui.dialog, {labeledElement: function (b, c, f, d) {
					if (!(
						...
						CKEDITOR.ui.dialog.uiElement.call(this, b, c, f, "div", null, {role: "presentation"}, function () {
							...
							if (c.labelLayout != "horizontal") {
								a.push('<label class="cke_dialog_ui_labeled_label' + f + '" ', ' id="' + h.labelId + '"', h.inputId ? ' for="' + h.inputId + '"' : "", (
									c.labelStyle ? ' style="' + c.labelStyle + '"' : ""
									) + ">", c.label, "</label>", '<div class="cke_dialog_ui_labeled_content"' + (
									c.controlStyle ? ' style="' + c.controlStyle + '"' : ""
									) + ' role="presentation">', d.call(this, b, c), "</div>");
							}
							else {
								f = {type: "hbox", widths: c.widths, padding: 0, children: [
									...
									{type: "html", html: '<span class="cke_dialog_ui_labeled_content"' + (
										c.controlStyle ? ' style="' + c.controlStyle + '"' : ""
										) + ">" + d.call(this, b, c) + "</span>"}
								]};
					...
					if (!(
						...
						CKEDITOR.ui.dialog.labeledElement.call(this, b, c, d, function () {
							...
							f(a);
							for (var m = 0, p; m < c.items.length && (
								p = c.items[m]
								); m++) {
								j.push('<option value="', CKEDITOR.tools.htmlEncode(p[1] !== void 0 ? p[1] : p[0]).replace(/"/g, "&quot;"), '" /> ', CKEDITOR.tools.htmlEncode(p[0]));
							}
							if (typeof a.inputStyle != "undefined") {
								a.style = a.inputStyle;
							}
					...
					CKEDITOR.ui.dialog.uiElement.call(this, a, d, f, "fieldset", null, null, function () {
						var a = [];
						h && a.push("<legend" + (
							d.labelStyle ? ' style="' + d.labelStyle + '"' : ""
							) + ">" + h + "</legend>");
						for (var b =
							0; b < c.length; b++) {
							a.push(c[b]);
				...
				CKEDITOR.ui.dialog.labeledElement.prototype = CKEDITOR.tools.extend(new CKEDITOR.ui.dialog.uiElement, {setLabel: function (a) {
					...
					b.getChildCount() < 1 ? (
						new CKEDITOR.dom.text(a, CKEDITOR.document)
						).appendTo(b) : b.getChild(0).$.nodeValue = a;
					return this
				}, getLabel: function () {
				...
				CKEDITOR.ui.dialog.radio.prototype = CKEDITOR.tools.extend(new CKEDITOR.ui.dialog.uiElement, {setValue: function (a, b) {
					for (var c = this._.children, f, d = 0; d < c.length && (
						f = c[d]
						); d++) {
						f.getElement().$.checked = f.getValue() == a;
					}
					!b && this.fire("change",
						{value: a})
				}, getValue: function () {
				...
				CKEDITOR.ui.dialog.file.prototype = CKEDITOR.tools.extend(new CKEDITOR.ui.dialog.labeledElement, h, {getInputElement: function () {
					...
					submit: function () {
						...
						for (d in a) {
							if (c = d.match(b)) {
								this.eventProcessors[d] ? this.eventProcessors[d].call(this, this._.dialog, a[d]) : f(this, this._.dialog, c[1].toLowerCase(), a[d]);
							}
						}
						return this
					}, reset: function () {
				...
				CKEDITOR.dialog.addUIElement("fieldset", {build: function (a, b, c) {
					for (var f = b.children, d, h = [], i = [], o = 0; o < f.length && (
						d = f[o]
						); o++) {
						var m =
							[];
			...
			(
				function () {
					...
					function h(a, b) {
						...
						c && (
							a ? c.removeAttribute("aria-invalid") : c.setAttribute("aria-invalid", true)
							);
						a || (
					...
					function i(a) {
						...
						if (CKEDITOR.env.ie && !CKEDITOR.env.ie6Compat) {
							var d = CKEDITOR.env.isCustomDomain(), d = "javascript:void(function(){" + encodeURIComponent("document.open();" + (
					...
					function g(a) {
						function b(c) {
							...
							d.x = d.x + m;
							d.y = d.y + o;
							a.move(d.x + h[3] < e ? -h[3] : d.x - h[1] > i.width - j.width - e ? i.width - j.width + (
								g.lang.dir == "rtl" ? 0 : h[1]
								) : d.x, d.y + h[0] < e ? -h[0] : d.y - h[2] > i.height - j.height - e ? i.height - j.height + h[2] : d.y, 1);
					...
					function j(a) {
						...
						function f(d) {
							var m = h.lang.dir == "rtl", o = n.width, p = n.height, q = o + (
								d.data.$.screenX - b
								) * (
								m ? -1 : 1
								) * (
								...
								a._.moved ? 1 : 2
								), u = a._.element.getFirst(), u = m && u.getComputedStyle("right"), s = a.getPosition();
							s.y + l > k.height && (
								l = k.height - s.y
								);
							if ((
						...
						if (e != CKEDITOR.DIALOG_RESIZE_NONE) {
							var h = a.getParentEditor(), j, i, k, n, m, o = CKEDITOR.tools.addFunction(function (g) {
								...
								i = n.height - a.parts.contents.getSize("height", !(
							...
							a.on("load", function () {
								var b = "";
								e == CKEDITOR.DIALOG_RESIZE_WIDTH ? b = " cke_resizer_horizontal" : e == CKEDITOR.DIALOG_RESIZE_HEIGHT && (
					...
					CKEDITOR.dialog = function (c, f) {
						...
						function n(f) {
							if (v ==
								...
								if (d == 9 || d == CKEDITOR.SHIFT + 9) {
									...
									d = f.data.getTarget();
									if (!d.is("a", "button", "select") && (
										!d.is("input") || d.$.type != "button"
										)) {
										(
											d = this.getButton("ok")
						...
						(
							l == "OS" && CKEDITOR.env.mac || l == "rtl" && u == "ltr" || l == "ltr" && u == "rtl"
							) && q.buttons.reverse();
						...
						if (c._.removeDialogTabs && (
							s = c._.removeDialogTabs[f]
							)) {
							for (u = 0; u < s.length; u++) {
								p.removeContents(s[u]);
							}
						}
						if (p.onLoad) {
						...
						if (p.onCancel) {
							this.on("cancel", function (a) {
								if (p.onCancel.call(this, a) === false) {
									a.data.hide = false
								}
							});
						}
						...
						this.on("cancel", function (a) {
							z(function (b) {
								if (b.isChanged()) {
									if (!confirm(c.lang.common.confirmCancel)) {
										a.data.hide =
											false;
									}
									return true
						...
						j(this);
						(
							new CKEDITOR.dom.text(p.title, CKEDITOR.document)
							).appendTo(this.parts.title);
						for (u = 0; u < p.contents.length; u++) {
							(
								s = p.contents[u]
								) && this.addPage(s);
						}
					...
					CKEDITOR.dialog.prototype = {destroy: function () {
						...
						return function (b, c, d) {
							...
							a === void 0 && (
								a = f.getComputedStyle("position") == "fixed"
								);
							if (!a || !this._.position || !(
								this._.position.x == b && this._.position.y == c
								)) {
								this._.position = {x: b, y: c};
								if (!a) {
									var g = CKEDITOR.document.getWindow().getScrollPosition(), b = b + g.x, c = c + g.y;
								}
								if (e) {
									g = this.getSize();
						...
						this.move(this._.moved ? this._.position.x : (
							a.width - b.width
							) / 2, this._.moved ? this._.position.y : (
							a.height - b.height
							) / 2)
					}, foreach: function (a) {
						...
						if (this.parts.dialog.isVisible()) {
							...
							for (J(this); CKEDITOR.dialog._.currentTop != this;) {
					...
					var u, v = {}, w, x = {}, y = function (a) {
						...
						(
							...
							).push({uiElement: a, dialog: b, key: c, keyup: f || a.accessKeyUp, keydown: d || a.accessKeyDown})
					}, J = function (a) {
						for (var b in x) {
							for (var c = x[b], d = c.length - 1; d >= 0; d--) {
								(
									c[d].dialog == a || c[d].uiElement == a
									) && c.splice(d, 1);
							}
						...
						function () {
							CKEDITOR.ui.dialog = {uiElement: function (a, b, c, d, f, e, g) {
								if (!(
									...
									for (var o = k["class"] &&
										k["class"].split ? k["class"].split(" ") : [], p = 0; p < o.length; p++) {
										o[p] && (
											m[o[p]] = 1
											);
									}
									...
									for (p in k) {
										j.push(p + '="' + CKEDITOR.tools.htmlEncode(k[p]) +
											'" ');
									}
									j.push(">", n, "</", h, ">");
									c.push(j.join(""));
								...
								if (!(
									...
									CKEDITOR.ui.dialog.uiElement.call(this, a, f || {type: "hbox"}, d, "table", {}, i, function () {
										...
										for (j = 0; j < c.length; j++) {
											...
											CKEDITOR.env.ie && (
												CKEDITOR.env.quirks && e[j].align
												) && d.push("text-align:" + e[j].align);
								...
								if (!(
									...
									CKEDITOR.ui.dialog.uiElement.call(this, a, f || {type: "vbox"}, d, "div", null, {role: "presentation"}, function () {
										...
										b.push('"');
										b.push('align="', CKEDITOR.tools.htmlEncode(f && f.align || (
											a.getParentEditor().lang.dir ==
												"ltr" ? "left" : "right"
											)), '" ');
										b.push("><tbody>");
										for (var d = 0; d < c.length; d++) {
											...
											CKEDITOR.env.ie && (
												CKEDITOR.env.quirks && e[d].align
												) && j.push("text-align:" + e[d].align);
					...
					(
						function () {
							var a = {build: function (a, b, c) {
								for (var d = b.children, f, e = [], g = [], h = 0; h < d.length && (
									f = d[h]
									); h++) {
									var j = [];
									e.push(j);
					...
					CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {openDialog: function (a, b) {
						...
						if (typeof d == "function") {
							d = this._.storedDialogs || (
								this._.storedDialogs = {}
								);
							d = d[a] || (
								d[a] = new CKEDITOR.dialog(this, a)
								);
							b && b.call(d, d);
							d.show();
							return d
				...
				function () {
					function a(a, b, c, d) {
						if (!a.isReadOnly() && !a.equals(c.editable())) {
							...
							if ((
								d ? a.getComputedStyle("direction") : a.getStyle("direction") || a.hasAttribute("dir")
								) != b) {
								a.removeStyle("direction");
								if (d) {
									a.removeAttribute("dir");
					...
					function c(c) {
						return{context: "p", refresh: function (a, b) {
							...
							if (!c) {
								d = b.lastElement;
								for (var e = a.editable(); d && !(
									d.getName()in f || d.equals(e)
									);) {
									var g = d.getParent();
									if (!g) {
										break;
							...
							if (d.equals(a.editable())) {
								(
									e = a.getSelection().getRanges()[0].getEnclosedNode()
							...
							if (h && h.length) {
								for (var j = {}, p = e.createBookmarks(), h = h.createIterator(), s, r = 0; s = h.getNextRange(1);) {
									...
									if (!t || t && !(
										t.type == CKEDITOR.NODE_ELEMENT && t.getName()in i
										)) {
										t = b(s, d, g);
									}
									t && a(t, c, f, j);
									...
									u.evaluator = function (a) {
										return!!(
											a.type == CKEDITOR.NODE_ELEMENT &&
												a.getName()in d && !(
												a.getName() == (
													g == CKEDITOR.ENTER_P ? "p" : "div"
													) && a.getParent().type == CKEDITOR.NODE_ELEMENT && a.getParent().getName() == "blockquote"
												) && a.getPosition(v) & CKEDITOR.POSITION_FOLLOWING && (
												a.getPosition(w) & CKEDITOR.POSITION_PRECEDING + CKEDITOR.POSITION_CONTAINS
									...
									s = s.createIterator();
									for (s.enlargeBr = g != CKEDITOR.ENTER_BR; t = s.getNextParagraph(g == CKEDITOR.ENTER_P ? "p" : "div");) {
										a(t, c, f, j)
									}
								}
					...
					for (var e = CKEDITOR.dom.element.prototype, g = ["setStyle", "removeStyle", "setAttribute", "removeAttribute"], j = 0; j < g.length; j++) {
						e[g[j]] = CKEDITOR.tools.override(e[g[j]], h)
					}
				}
				)();
			(
					...
					var a = {exec: function (a) {
						...
						if (d) {
							...
							if (c == CKEDITOR.TRISTATE_OFF) {
								...
								for (c = {table: 1, tbody: 1, tr: 1, ol: 1, ul: 1}; c[g.getName()];) {
									g = g.getParent();
								}
								for (e = null; f.length > 0;) {
									for (c = f.shift(); !c.getParent().equals(g);) {
										c = c.getParent();
									}
									c.equals(e) || d.push(c);
									e = c
								...
								for (g = {}; e.length > 0;) {
									...
									if (j.getPrevious()) {
										if (j.getNext()) {
											...
											j.remove().insertAfter(d);
										}
									}
									else {
										j.remove().insertBefore(d);
									}
								...
								for (e = f.length - 1; e >= 0; e--) {
									...
									a:{
										g = d;
										for (var j = 0, k = g.getChildCount(), q = void 0; j < k && (
											q = g.getChild(j)
											); j++) {
								...
								if (a.config.enterMode == CKEDITOR.ENTER_BR) {
									for (d = true; c.length;) {
										...
										if (j.getName() == "div") {
											...
											d && (
												j.getPrevious() && !(
													j.getPrevious().type == CKEDITOR.NODE_ELEMENT && j.getPrevious().isBlockBoundary()
													)
												) && e.append(a.document.createElement("br"));
											for (d = j.getNext() && !(
												j.getNext().type ==
													CKEDITOR.NODE_ELEMENT && j.getNext().isBlockBoundary()
												); j.getFirst();) {
												j.getFirst().remove().appendTo(e);
											}
				...
				function () {
					function a(a) {
						...
						function l(b) {
							...
							d.on(b, g);
							(
								CKEDITOR.env.version > 7 ? c.$ : c.$.selection.createRange()
								).execCommand(b);
							...
							if (!d.getById("cke_pastebin")) {
								...
								if (h) {
									...
									if (CKEDITOR.env.webkit) {
										e.append(k);
										p = (
											e.is("body") ? e : CKEDITOR.dom.element.get(k.$.offsetParent)
											).getDocumentPosition().y
									}
									else {
										e.getAscendant(CKEDITOR.env.ie || CKEDITOR.env.opera ? "body" : "html", 1).append(k);
									}
								...
								if (h = k.getParent().isReadOnly()) {
									...
									k.setStyle(a.config.contentsLangDirection == "ltr" ? "left" : "right", "-1000px");
								}
								...
								setTimeout(function () {
									...
									var b;
									k = CKEDITOR.env.webkit && (
										b = k.getFirst()
										) && b.is && b.hasClass("Apple-style-span") ? b : k;
									j.selectBookmarks(i);
						...
						function t(b) {
							...
							if (b == "Paste") {
								...
								b = a.getSelection();
								c = b.getRanges();
								c = b.type != CKEDITOR.SELECTION_NONE && !(
									c.length == 1 && c[0].collapsed
									)
							}
						...
						a.getClipboardData = function (b, c) {
							...
							if (CKEDITOR.env.ie) {
								a.focus();
								h();
								a.editable().fire(x) && !l("paste") && (
									m = false
									)
							}
							else {
								try {
									if (a.editable().fire(x) && !a.document.$.execCommand("Paste", false, null)) {
										throw 0;
									}
								} catch (o) {
									m = false
					...
					function i(a, b) {
						...
						}).replace(/<\/?p>/g, "") : a.enterMode == CKEDITOR.ENTER_DIV && (
					...
					CKEDITOR.plugins.add("clipboard", {requires: "dialog", init: function (f) {
						...
						f.on("paste", function (a) {
							...
							q == "htmlifiedtext" ? i = c(f.config, i) : j == "text" && q == "html" && (
								i = d(f.config, i, e || (
									e = h(f)
									))
								);
							a.startsWithEOL && (
								i = '<br data-cke-eol="1">' + i
								);
							a.endsWithEOL && (
								i = i + '<br data-cke-eol="1">'
								);
							j == "auto" && (
								j = q == "html" || l == "html" ? "html" : "text"
								);
							a.type = j;
							a.dataValue = i;
			...
			(
				function () {
					var a = '<a id="{id}" class="cke_button cke_button__{name} cke_button_{state}"' + (
					...
					var a = a + (
							' onkeydown="return CKEDITOR.tools.callFunction({keydownFn},event);" onfocus="return CKEDITOR.tools.callFunction({focusFn},event);" ' + (
								...
								) + '="CKEDITOR.tools.callFunction({clickFn},this);return false;"><span class="cke_button_icon cke_button__{name}_icon" style="{style}"'
							), a = a + '>&nbsp;</span><span id="{id}_label" class="cke_button_label cke_button__{name}_label">{label}</span>{arrowHtml}</a>',
						b = CKEDITOR.addTemplate("buttonArrow", '<span class="cke_button_arrow">' + (
							CKEDITOR.env.hc ? "&#9660;" : "&nbsp;"
							) + "</span>"), c = CKEDITOR.addTemplate("button", a);
					...
					CKEDITOR.ui.button.prototype = {render: function (a, d) {
						...
						if (this.modes) {
							...
							if (g = a.getCommand(g)) {
								...
								e = e + (
									g.state == CKEDITOR.TRISTATE_ON ? "on" : g.state == CKEDITOR.TRISTATE_DISABLED ? "disabled" : "off"
									)
							}
						}
						g || (
						...
						i = {id: f, name: o, label: this.label, state: e, title: this.title, titleJs: i.gecko && i.version >= 10900 && !i.hc ? "" : (
							this.title || ""
							).replace("'",
			...
			(
				function () {
					...
					CKEDITOR.ui.panel.prototype = {render: function (h, d) {
								...
								if (this.isFramed) {
									...
									a.on("key" + (
										...
										this._.onKeyDown && this._.onKeyDown(b) === false ? a.data.preventDefault() : (
											...
											) && this.onEscape && this.onEscape(b) === false && a.data.preventDefault()
									}, this);
									a = a.getBody();
									a.unselectable();
						...
						if (this.isFramed) {
							i.frame = b.output({id: this.id + "_frame", src: "javascript:void(document.open()," + (
					...
					CKEDITOR.ui.panel.block = CKEDITOR.tools.createClass({$: function (a, b) {
						...
						(
							!this.onHide || this.onHide.call(this) !== true
						...
						switch (b) {
							...
							case "prev":
								a = this._.focusIndex;
								for (b = this.element.getElementsByTag("a"); a > 0 && (
									c = b.getItem(--a)
									);) {
									if (c.getAttribute("_cke_focus") && c.$.offsetWidth) {
										this._.focusIndex = a;
							...
							case "mouseup":
								a = this._.focusIndex;
								(
									c = a >= 0 && this.element.getElementsByTag("a").getItem(a)
									) && (
									c.$[b] ? c.$[b]() : c.$["on" + b]()
									);
								return false
				...
				function () {
					...
					CKEDITOR.ui.floatPanel = CKEDITOR.tools.createClass({$: function (b, h, d, i) {
						...
						var j = this.element, a = this._.iframe,
							k = b.getDocumentPosition(j.getDocument()), q = this._.dir == "rtl", l = k.x + (
								i || 0
								), n = k.y + (
								f || 0
								);
						...
						CKEDITOR.tools.setTimeout(function () {
							var a = CKEDITOR.tools.bind(function () {
								...
								q ? f < c && (
									l = h > c ? l + c : b.width > c ? l - a.left : l - a.right + b.width
									) : f < c && (
									l = h > c ? l - c : b.width > c ? l - a.right + b.width : l - a.left
									);
								c = a.top;
								b.height - a.top < d && (
									n = c > d ? n - d : b.height > d ? n - a.bottom + b.height : n - a.top
									);
								if (CKEDITOR.env.ie) {
									...
									b.getComputedStyle("direction") == "rtl" && (
										l = CKEDITOR.env.ie8Compat ? l - j.getDocument().getDocumentElement().$.scrollLeft * 2 : l - (
								...
								var a = j.getFirst(), i;
								(
									i = a.getCustomData("activePanel")
									) && i.onHide && i.onHide.call(this, 1);
						...
						if (!(
							this._.activeChild == a && a._.panel._.offsetParentId ==
								d.getId()
							)) {
							this.hideChild();
			...
			CKEDITOR.plugins.add("colorbutton", {requires: "panelbutton,floatpanel", init: function (a) {
				...
				function c(b, c, g) {
					...
					(
						a.plugins.colordialog && d.colorButton_enableMore === void 0 || d.colorButton_enableMore
						) && j.push('</tr><tr><td colspan=8 align=center><a class="cke_colormore" _cke_focus=1 hidefocus=true title="',
			...
			CKEDITOR.plugins.add("menu",
				{beforeInit: function (a) {
					for (var b = a.config.menu_groups.split(","), c = a._.menuGroups = {}, h = a._.menuItems = {}, d = 0; d < b.length; d++) {
						c[b[d]] = d + 1;
					}
					...
					a.addMenuItem = function (a, b) {
						c[b.group] && (
							h[a] = new CKEDITOR.menuItem(this, a, b)
							)
					};
			...
			(
				function () {
					...
					CKEDITOR.env.gecko && (
						b = b + ' onblur="this.style.cssText = this.style.cssText;"'
						);
					var b = b + (
					...
					CKEDITOR.menu = CKEDITOR.tools.createClass({$: function (a, b) {
						...
						for (var g = 0; g < e.length; g++) {
							...
							if (h) {
								for (var k in h) {
									...
									if (q && (
										!q.command || this.editor.getCommand(q.command).state
										)) {
										q.state = h[k];
										this.add(q)
									}
						...
						var c = c || (
							this.editor.lang.dir == "rtl" ? 2 : 1
							), g = this.items, h = this.editor, k = this._.panel, q = this._.element;
						if (!k) {
						...
						a(g);
						for (var l = h.container && h.container.getChild(1), l = ['<div class="cke_menu' + (
							l && l.hasClass("cke_mixed_dir_content") ? " cke_mixed_dir_content" : ""
							) + '" role="presentation">'], n = g.length, o = n && g[0].group, m = 0; m < n; m++) {
							var p = g[m];
							if (o != p.group) {
					...
					CKEDITOR.menuItem = CKEDITOR.tools.createClass({$: function (a, b, c) {
						...
						var e = typeof this.state ==
							"undefined" ? CKEDITOR.TRISTATE_OFF : this.state, g = this.getItems, j = "&" + (
							this.editor.lang.dir == "rtl" ? "9668" : "9658"
							) + ";", a = {id: a.id + (
							"" + b
							), name: this.name, label: this.label, state: e == CKEDITOR.TRISTATE_ON ? "on" : e == CKEDITOR.TRISTATE_DISABLED ? "disabled" : "off", hasPopup: g ? "true" : "false", disabled: e == CKEDITOR.TRISTATE_DISABLED, pressed: e == CKEDITOR.TRISTATE_ON, title: this.label, href: "javascript:void('" + (
							this.label || ""
							).replace("'") + "')", hoverFn: a._.itemOverFn, moveOutFn: a._.itemOutFn, clickFn: a._.itemClickFn, index: b, iconStyle: CKEDITOR.skin.getIconStyle(this.name,
			...
			CKEDITOR.plugins.add("contextmenu", {requires: "menu", onLoad: function () {
				CKEDITOR.plugins.contextMenu = CKEDITOR.tools.createClass({base: CKEDITOR.menu, $: function (a) {
					...
					if (CKEDITOR.env.opera && !(
						"oncontextmenu"in document.body
						)) {
						var c;
						a.on("mousedown", function (d) {
							d = d.data;
							if (d.$.button != 2) {
								d.getKeystroke() == CKEDITOR.CTRL + 1 && a.fire("contextmenu", d);
							}
							else if (!b || !(
								CKEDITOR.env.mac ? d.$.metaKey : d.$.ctrlKey
								)) {
								var f = d.getTarget();
								if (!c) {
									f = f.getDocument();
								...
								c.setAttribute("style", "position:absolute;top:" + (
									d.$.clientY - 2
									) + "px;left:" +
									(
										d.$.clientX - 2
					...
					a.on("contextmenu", function (a) {
						a = a.data;
						if (!b || !(
							CKEDITOR.env.webkit ? h : CKEDITOR.env.mac ? a.$.metaKey : a.$.ctrlKey
							)) {
							a.preventDefault();
					...
					CKEDITOR.plugins.add("div", {requires: "dialog", init: function (a) {
						if (!a.blockless) {
							...
							a.addCommand("creatediv", new CKEDITOR.dialogCommand("creatediv", {contextSensitive: true, refresh: function (a, b) {
								this.setState("div"in(
									a.config.div_wrapTable ? b.root : b.blockLimit
							...
							a.addCommand("removediv", {exec: function (a) {
								function b(d) {
									if ((
										d = CKEDITOR.plugins.div.getSurroundDiv(a, d)
			...
			(
				function () {
					var a = {editorFocus: false, readOnly: 1, exec: function (a) {
						...
						c = "";
					if (CKEDITOR.env.opera || CKEDITOR.env.gecko && CKEDITOR.env.mac) {
						c = c + ' onkeypress="return false;"';
					}
					CKEDITOR.env.gecko && (
						c = c + ' onblur="this.style.cssText = this.style.cssText;"'
						);
					var h = CKEDITOR.addTemplate("pathItem", '<a id="{id}" href="{jsTitle}" tabindex="-1" class="cke_path_item" title="{label}"' + (
							...
							switch (b.getKeystroke()) {
								...
								case CKEDITOR.SHIFT + 9:
									(
										...
										g = CKEDITOR.document.getById(f + (
											c._.elementsPath.list.length - 1
											))
										);
									g.focus();
									return false;
			...
			(
				function () {
					function a(a, b, c) {
						function d(c) {
							if ((
								i = j[c ? "getFirst" : "getLast"]()
								) && (
								!i.is || !i.isBlockBoundary()
								) && (
								k = b.root[c ? "getPrevious" : "getNext"](CKEDITOR.dom.walker.invisible(true))
								) && (
								!k.is || !k.isBlockBoundary({br: 1})
								)) {
					...
					function c(a, b, c, d) {
						for (var f, g; f = a[d ? "getLast" : "getFirst"](o);) {
							(
								g = f.getDirection(1)
								) !== b.getDirection(1) && f.setAttribute("dir", g);
							f.remove();
					...
					function d(a) {
						return function (b) {
							...
							if (d && (
								d.name && d.name == "br" || d.value && (
									e = d.value.match(p)
									)
								)) {
								(
									!e || !e.index
									) && d == c[0] ? c[0] = a || CKEDITOR.env.ie ? new CKEDITOR.htmlParser.text(" ") : new CKEDITOR.htmlParser.element("br", {}) : d.name == "br" ? c.splice(b - 1, 1) : d.value = d.value.replace(p, "")
							}
						}
					}
					...
					function f(a, b, d) {
						...
						(
							...
							j.type == CKEDITOR.NODE_ELEMENT && j.is("br")
							) && j.remove();
						(
							j = b.startContainer.getChild(b.startOffset)
							) ? f.insertBefore(j) : b.startContainer.append(f);
						f = d.startPath();
						if (f = f.contains("li")) {
							if (j = e(f)) {
								if (i.contains(f)) {
						...
						for (; d.checkStartOfBlock() && d.checkEndOfBlock();) {
							...
							if (i.is("li")) {
								f = i.getParent();
								i.equals(f.getLast(q)) && i.equals(f.getFirst(q)) && (
									i = f
									)
							}
						...
						(
							...
							d.type == CKEDITOR.NODE_ELEMENT && d.getName()in CKEDITOR.dtd.$list
							) && h(d);
						b.moveToBookmark(g);
						b.select();
						a.fire("saveSnapshot")
					...
					CKEDITOR.plugins.list = {listToArray: function (a, b, c, d, f) {
						...
						for (var e = 0, h = a.getChildCount(); e < h; e++) {
							...
							if (j.$.nodeName.toLowerCase() ==
								...
								if (f) {
									...
									if (i.grandparent && i.grandparent.$.nodeName.toLowerCase() == "li") {
										i.grandparent = i.grandparent.getParent()
									}
								}
						...
						for (var e, h = a[c].parent.getDocument(), j = new CKEDITOR.dom.documentFragment(h), i = null, k = c, m = Math.max(a[c].indent, 0), p = null, n, o, l = d == CKEDITOR.ENTER_P ? "p" : "div"; ;) {
							...
							if (s.indent == m) {
								...
								for (e = 0; e < s.contents.length; e++) {
									p.append(s.contents[e].clone(1,
										1));
								}
								k++
							}
								...
								if (g[e.getName()]) {
									...
									p = new CKEDITOR.dom.documentFragment(h);
								}
								var i = e.getDirection(1) != n, r = s.element, t = r.getAttribute("class"), O = r.getAttribute("style"),
									H = p.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT && (
										d != CKEDITOR.ENTER_BR || i || O || t
								...
								for (e = 0; e < Q; e++) {
									...
									if (M.type == CKEDITOR.NODE_ELEMENT && M.isBlockBoundary()) {
										...
										R && N.setAttribute("style", R.replace(/([^;])$/, "$1;") + (
								...
								if (p.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT && k != a.length - 1) {
									(
										...
										n.type == CKEDITOR.NODE_ELEMENT && n.getAttribute("type") == "_moz"
										) && n.remove();
									(
										!p.getLast(q) || !(
											n.type == CKEDITOR.NODE_ELEMENT && n.getName()in CKEDITOR.dtd.$block
											)
										) && p.append(h.createElement("br"))
								}
							...
							o = null;
							if (a.length <= k || Math.max(a[k].indent, 0) < m) {
								break
							}
						}
						if (b) {
						...
						for (var c = d.createBookmarks(true), e = [], i = {}, f = f.createIterator(), k = 0; (
							...
							for (j.forceBrBreak = this.state == CKEDITOR.TRISTATE_OFF; p = j.getNextParagraph();) {
								if (!p.getCustomData("list_block")) {
									...
									for (var l = b.elementPath(p), m = l.elements, o = 0, l = l.blockLimit, s, r = m.length - 1; r >= 0 && (
										s = m[r]
										); r--) {
										if (g[s.getName()] && l.contains(s)) {
											l.removeCustomData("list_group_object_" + k);
											if (m = s.getCustomData("list_group_object")) {
												m.contents.push(p);
											}
											else {
												m = {root: s, contents: [p]};
									...
									if (!o) {
										o = l;
										if (o.getCustomData("list_group_object_" + k)) {
											o.getCustomData("list_group_object_" + k).contents.push(p);
										}
										else {
											m = {root: o, contents: [p]};
								...
								if (g[m.root.getName()]) {
									...
									for (j = 0; j < f.contents.length; j++) {
										...
										if ((
											...
											) && !r.getCustomData("list_item_processed")) {
											l.push(r);
									...
									for (j = 0; j < o && (
										m = p.listNode.getChild(j)
										); j++) {
										m.getName() == this.type && k.push(m);
									}
									...
									for (r = 0; r < l.length; r++) {
										p = p.getCommonAncestor(l[r].getParent());
									}
								...
								if (a.mode == "wysiwyg" && c in{8: 1, 46: 1}) {
									...
									if (d.collapsed) {
										...
										if (j) {
											...
											if ((
												m = h.contains(g)
												) && d.checkBoundaryOfElement(m, CKEDITOR.START) && (
												m = m.getParent()
												) && m.is("li") && (
												m = e(m)
												)) {
											...
											if ((
												k = p.next()
												) && k.type == CKEDITOR.NODE_ELEMENT && k.getName()in g && k.equals(h)) {
												n = 1;
												k = p.next()
											}
											else d.checkBoundaryOfElement(m, CKEDITOR.END) && (
												n = 1
												);
											if (n && k) {
												d = d.clone();
			...
			(
				function () {
					...
					function b(a, b) {
						return(
							b || a.getComputedStyle("direction")
							) ==
					...
					a.prototype = {context: "p", refresh: function (a, c) {
							...
							if (c && e) {
								...
								if (!(
									...
									if (k.name == "outdent") {
										...
										if ((
											n = b.getParent()
											) && n.is("li"))for (var g = c.listNode.getChildren(), l = [], r, e = g.count() - 1; e >= 0; e--)(
											r = g.getItem(e)
											) && (
											r.is && r.is("li")
											) && l.push(r)
						...
						function j(c, d) {
							...
							if (k.useIndentClasses) {
								...
								var h = a.config.indentOffset || 40, g = g + (
									k.name == "indent" ? 1 : -1
									) * h;
								if (g < 0)return false;
								g = Math.max(g, 0);
						...
						for (var k = this, q = {}, l = a.getSelection(), n = l.createBookmarks(1), o, m = (
							l && l.getRanges(1)
							).createIterator(); o = m.getNextRange();) {
							for (var p = o.getCommonAncestor(); p && !(
								p.type == CKEDITOR.NODE_ELEMENT && h[p.getName()]
								);)p = p.getParent();
							if (!p) {
							...
							if (p) {
								...
								(
									!s.equals(t) && !s.contains(t) || !(
										k.name == "indent" || k.useIndentClasses || parseInt(p.getStyle(b(p)), 10)
								...
								if (!n && !o) {
									if (a.is("li")) {
										...
										q.evaluator = function (a) {
											return!(
												h(a) || c(a) || a.type == CKEDITOR.NODE_ELEMENT && a.getName()in CKEDITOR.dtd.$inline && !(
													a.getName()in CKEDITOR.dtd.$empty
													)
												)
										};
										(
											...
											s.type == CKEDITOR.NODE_ELEMENT && s.is("ul", "ol")
											) && (
											CKEDITOR.env.ie ? l.createText(" ") : l.createElement("br")
											).insertBefore(s)
									}
				...
				function () {
					...
					CKEDITOR.plugins.add("entities", {afterInit: function (b) {
						...
						if (b = (
							...
							delete d.regex;
							c.entities && c.entities_processNumerical && (
								i = "[^ -~]|" + i
								);
			...
			CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {popup: function (a, b, c, h) {
				...
				c = c || "70%";
				typeof b == "string" && (
					b.length > 1 && b.substr(b.length - 1, 1) == "%"
					) && (
					b = parseInt(window.screen.width * parseInt(b, 10) / 100, 10)
					);
				typeof c == "string" && (
					c.length > 1 && c.substr(c.length - 1, 1) == "%"
					) && (
				...
				var d = parseInt((
					window.screen.height - c
					) / 2, 10), i = parseInt((
					window.screen.width - b
					) / 2, 10), h = (
					h || "location=no,menubar=no,toolbar=no,dependent=yes,minimizable=no,modal=yes,alwaysRaised=yes,resizable=yes,scrollbars=yes"
					) + ",width=" + b + ",height=" + c + ",top=" + d + ",left=" + i, f = window.open("", null, h, true);
				if (!f)return false;
				try {
				...
				function () {
					...
					function i(a, f, e, q) {
						...
						for (n in q) {
							l = q[n];
							(
								l.type == "hbox" || l.type == "vbox" || l.type == "fieldset"
								) && i(a, f, e, l.children);
							if (l.filebrowser) {
					...
					function f(a, b, c) {
						...
						return(
							a = a.getContents(b).get(c).filebrowser
							) && a.url
					}
					...
					function e(a, b) {
						...
						if (!(
							typeof b == "function" && b.call(this._.filebrowserSe) === false
							) && !(
							f && f.call(this._.filebrowserSe, a, b) === false
							)) {
				...
				function () {
					...
					var b = CKEDITOR.htmlParser.cssStyle, c = CKEDITOR.tools.cssLength, h = /^((?:\d*(?:\.\d+))|(?:\d+))(.*)?$/i, d = {elements: {$: function (c) {
						...
						if ((
							d = (
								d = (
									d = d && d["data-cke-realelement"]
									) && new CKEDITOR.htmlParser.fragment.fromHtml(decodeURIComponent(d))
								) && d.children[0]
							) &&
							c.attributes["data-cke-resizable"]) {
							var g = (
								new b(c)
								).rules, c = d.attributes, h = g.width, g = g.height;
							h && (
								c.width = a(c.width, h)
								);
							g && (
								c.height = a(c.height, g)
								)
						}
						return d
					...
					CKEDITOR.editor.prototype.createFakeElement = function (a, d, g, h) {
						...
						g && (
							d["data-cke-real-element-type"] = g
							);
						if (h) {
				...
				function () {
					...
					CKEDITOR.plugins.add("flash", {requires: "dialog,fakeobjects", onLoad: function () {
						...
						afterInit: function (c) {
							...
							(
								d = d && d.dataFilter
								) && d.addRules({elements: {"cke:object": function (d) {
								var f = d.attributes;
								if ((
									!f.classid || !(
										"" + f.classid
				...
				function () {
					...
					function b(b) {
						var f, e = b.config, g = e.floatSpaceDockedOffsetX || 0, j = e.floatSpaceDockedOffsetY || 0, k = e.floatSpacePinnedOffsetX || 0, q = e.floatSpacePinnedOffsetY || 0, l = function (c) {
							...
							if (f) {
								f == "top" && v.top < q ? n("pin") : f == "pin" ? w.top > j + x ? n("top") : w.bottom - v.bottom < x && n("bottom") : f == "bottom" && (
									w.top > j + x ? n("top") : w.bottom > 2 * x + q && n("pin")
									);
								if (o) {
									c = h.getViewPaneSize();
									o = c.width / 2;
									...
									m.setStyle(v, d((
										...
										v == "left" ? w.left > 0 ? w.left : 0 : w.right < c.width ? c.width - w.right : 0
										) + y))
								}
							}
							else {
					...
					var c = CKEDITOR.addTemplate("floatcontainer", '<div id="cke_{name}" class="cke {id} cke_reset_all cke_chrome cke_editor_{name} cke_float cke_{langDir} ' + CKEDITOR.env.cssClass +
						'" dir="{langDir}" title="' + (
						CKEDITOR.env.gecko ? " " : ""
						) + '" lang="{langCode}" role="presentation" style="{style}"><div class="cke_inner"><div id="{topId}" class="cke_top" role="presentation">{content}</div></div></div>');
			...
			(
				function () {
					...
					CKEDITOR.ui.richCombo = CKEDITOR.tools.createClass({$: function (a) {
						...
						g.keyDownFn = j;
						i = {id: f, name: this.name || this.command, label: this.label, title: this.title, titleJs: i.gecko && i.version >= 10900 && !i.hc ? "" : (
							this.title || ""
							).replace("'",
			...
			CKEDITOR.plugins.add("forms", {requires: "dialog,fakeobjects", onLoad: function () {
				...
				var b = a.lang, c = 0, h = function (d, e, g) {
					...
					a.ui.addButton && a.ui.addButton(d, {label: b.common[d.charAt(0).toLowerCase() + d.slice(1)], command: e, toolbar: "forms," + (
						c = c + 10
						)});
				...
				if (a.addMenuItems) {
					...
					!a.blockless && (
						h.form = {label: b.forms.form.menu, command: "form", group: "form"}
						);
					a.addMenuItems(h)
				}
				if (a.contextMenu) {
					!a.blockless && a.contextMenu.addListener(function (a, b, c) {
						if ((
							a = c.contains("form",
								1)
							) && !a.isReadOnly())return{form: CKEDITOR.TRISTATE_OFF}
					});
			...
			CKEDITOR.env.ie && (
				CKEDITOR.dom.element.prototype.hasAttribute = CKEDITOR.tools.override(CKEDITOR.dom.element.prototype.hasAttribute, function (a) {
					return function (b) {
			...
			CKEDITOR.htmlWriter = CKEDITOR.tools.createClass({base: CKEDITOR.htmlParser.basicWriter, $: function () {
				...
				for (b in CKEDITOR.tools.extend({}, a.$nonBodyContent, a.$block, a.$listItem, a.$tableContent))this.setRules(b, {indent: !a[b]["#"],
					breakBeforeOpen: 1, breakBeforeClose: !a[b]["#"], breakAfterClose: 1, needsSpace: b in a.$block && !(
						b in{li: 1, dt: 1, dd: 1}
						)});
				...
				this._.afterCloser && (
					b && b.needsSpace && this._.needsSpace
					) && this._.output.push("\n");
			...
			(
				function () {
					CKEDITOR.plugins.add("iframe", {requires: "dialog,fakeobjects", onLoad: function () {
						...
						(
							b = b && b.dataFilter
							) && b.addRules({elements: {iframe: function (b) {
						...
						if (c) {
							for (var f = c.createBookmarks(), e = c.getRanges(true), g = this.cssClassName, j, k, q = b.config.useComputedState,
								...
								for (j.enlargeBr = i != CKEDITOR.ENTER_BR; k = j.getNextParagraph(i == CKEDITOR.ENTER_P ? "p" : "div");) {
									...
									var n = g && (
										k.$.className = CKEDITOR.tools.ltrim(k.$.className.replace(this.cssClassRegex, ""))
										), o = this.state == CKEDITOR.TRISTATE_OFF && (
										!q || a(k, true) != this.value
										);
			...
			CKEDITOR.plugins.add("link", {requires: "dialog,fakeobjects",
				onLoad: function () {
					...
					var b = "background:url(" + CKEDITOR.getUrl(this.path + "images/anchor.gif") + ") no-repeat %1 center;border:1px dotted #00f;", c = ".%2 a.cke_anchor,.%2 a.cke_anchor_empty" + (
						CKEDITOR.env.ie && CKEDITOR.env.version < 7 ? "" : ",.cke_editable.%2 a[name],.cke_editable.%2 a[data-cke-saved-name]"
						) + "{" + b + "padding-%1:18px;cursor:auto;}" + (
						CKEDITOR.env.ie ? "a.cke_anchor_empty{display:inline-block;}" : ""
						) + ".%2 img.cke_anchor{" +
						b + "width:16px;min-height:15px;height:1.15em;vertical-align:" + (
					...
					a.on("doubleclick", function (b) {
						...
						if (!c.isReadOnly())if (c.is("a")) {
							b.data.dialog = c.getAttribute("name") && (
					...
					a.contextMenu.addListener(function (b) {
						...
						if (!b && !(
							b = CKEDITOR.plugins.link.getSelectedLink(a)
							))return null;
						var c = {};
					...
					h && h.push(function (b, c) {
						if (c == "a" && (
							CKEDITOR.plugins.link.tryRestoreFakeAnchor(a, b) || b.getAttribute("name") && (
								!b.getAttribute("href") || !b.getChildCount()
								)
							))return"anchor"
					})
			...
			CKEDITOR.removeAnchorCommand.prototype = {exec: function (a) {
				...
				if (b && (
					...
					CKEDITOR.plugins.link.fakeAnchor && !h.getChildCount() ? CKEDITOR.plugins.link.tryRestoreFakeAnchor(a,
						h) : h.is("a")
					))h.remove(1);
			...
			(
				function () {
					...
					function c(a, b) {
						...
						var c;
						return j(b) ? (
							c = b.getAscendant(a.triggers, true)
							) && !c.contains(a.editable) ? c : null : null
					}
					...
					function f(a) {
						...
						C(c, {attach: function () {
							...
							E + "</span>", b), {base: K + "height:17px;width:17px;" + (
							a.rtl ? "left" : "right"
							) + ":17px;background:url(" + this.path + "images/icon.png) center no-repeat " + a.boxColor + ";cursor:" + (
							H.opera ? "auto" : "pointer"
							) + ";", looks: ["top:-8px;" + CKEDITOR.tools.cssVendorPrefix("border-radius", "2px", 1), "top:-17px;" + CKEDITOR.tools.cssVendorPrefix("border-radius", "2px 2px 0px 0px", 1), "top:-1px;" + CKEDITOR.tools.cssVendorPrefix("border-radius", "0px 0px 2px 2px", 1)]}), C(O(I, b), {base: L + "left:0px;border-left-color:" + a.boxColor + ";", looks: ["border-width:8px 0 8px 8px;top:-8px",
							...
							if (h.equals(c)) {
								...
								j.left = g.size.left - g.size.margin.left + b.scroll.x - (
									i(a) ? b.editable.left + b.editable.border.left : 0
									);
					...
					function m(a) {
						...
						if (f && c.contains(f))for (; !f.getParent().equals(c);)f = f.getParent();
						else f = c.getLast();
						if (!d || !f)return null;
						if (!j(d) || g(a, d))if (!(
							d = d.getNext(a.isRelevant)
							))return null;
						r(a, d);
					...
					function p(a) {
						...
						if (!k(d) && !k(c))if (b.is(y)) {
							if (c && d && !d.equals(c) && !c.equals(d) && !d.contains(c) && !c.contains(d) && l(a, c) && (
								l(a, d) && j(c) && j(d) && d.equals(c.getNext(a.isRelevant))
								) && (
								b.is(D) || b.is(z) && !n(a, c, true) && !n(a, d, false)
								))return true
						}
						else if (b.is(w)) {
					...
					function s(a, b, c, d) {
						...
						i.height = i.outerHeight - (
							j.top + j.bottom + g.top +
								g.bottom
							);
						i.width = i.outerWidth - (
							j.left + j.right + g.left + g.right
							);
					...
					CKEDITOR.plugins.add("magicline", {init: function (e) {
						var h = e.config, n = h.magicline_triggerOffset || 30, l = {editor: e, triggerOffset: n, holdDistance: 0 | n * (
							h.magicline_holdDistance || 0.5
							), boxColor: h.magicline_color || "#ff0000", rtl: h.contentsLangDirection == "rtl", triggers: h.magicline_everywhere ? CKEDITOR.dtd.$block : {table: 1, hr: 1, div: 1, ul: 1, ol: 1, dl: 1}}, v, y, D, A;
								...
								h.attachListener(k, "mouseout", function (a) {
									if (e.mode == "wysiwyg") {
										...
										if (i(l)) {
											...
											if (!(
												b > c.left - d.x && b < c.right - d.x
												) || !(
												a > c.top - d.y && a < c.bottom - d.y
												)) {
												clearTimeout(A);
												A = null;
												l.line.detach()
								...
								h.attachListener(i(l) ? h : k, "mousemove", function (c) {
									...
									if (!(
										...
										A = setTimeout(function () {
											...
											A = l.trigger = null;
											u(l);
											if (D && !l.hiddenMode && e.focusManager.hasFocus && !l.line.mouseNear() && (
												l.element = b(l, true)
												)) {
												var c;
												var f = l.editable;
			...
			(
				function () {
					...
					function b(a, b) {
						if (a && !(
							a.type != CKEDITOR.NODE_ELEMENT || a.getName() != "form"
							) && b.length > 0)for (var c = b.length - 1; c >= 0; c--) {
							var d = b[c][0], h = b[c][1];
					...
					function i(a) {
						...
						var b =
							CKEDITOR.dom.element.createFromHtml('<iframe frameborder="0" tabindex="-1" src="javascript:void((function(){document.open();' + (
								CKEDITOR.env.isCustomDomain() ? "document.domain='" + this.getDocument().$.domain + "';" : ""
								) + 'document.close();})())" style="display:block;position:absolute;z-index:-1;progid:DXImageTransform.Microsoft.Alpha(opacity=0);"></iframe>');
						...
						if (a.elementMode != CKEDITOR.ELEMENT_MODE_INLINE) {
							...
							a.addCommand("maximize", {modes: {wysiwyg: !CKEDITOR.env.iOS, source: !CKEDITOR.env.iOS}, readOnly: 1, editorFocus: false, exec: function () {
								...
								if (a.mode == "wysiwyg")if (q) {
									...
									(
										t = a.getSelection().getStartElement()
			...
			CKEDITOR.plugins.add("pagebreak", {onLoad: function () {
				...
				(
					c = c && c.htmlFilter
					) && c.addRules({attributes: {"class": function (a, b) {
				...
				a && a.addRules({elements: {div: function (a) {
					...
					if ((
						f = f && f.name == "span" && f.attributes.style
						) &&
				...
				function () {
					...
					CKEDITOR.plugins.add("pastefromword", {requires: "clipboard", init: function (c) {
						...
						c.on("paste", function (b) {
							...
							if (e && (
								h || /(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/.test(e)
								)) {
								var g = a(d, function () {
									if (g)c.fire("paste",
										f);
				...
				function () {
					var a, b = {modes: {wysiwyg: 1, source: 1}, canUndo: false, readOnly: 1, exec: function (b) {
						...
						if (h.fullPage)b =
							...
							b = b.config.docType + '<html dir="' + b.config.contentsLangDirection + '"><head>' + d + "<title>" + b.lang.preview.preview + "</title>" + CKEDITOR.tools.buildStyleHtml(b.config.contentsCss) + "</head>" + (
								h + ">"
								) + b.getData() +
								"</body></html>"
						}
			...
			CKEDITOR.plugins.removeformat = {commands: {removeformat: {exec: function (a) {
				for (var b = a._.removeFormatRegex || (
						a._.removeFormatRegex = RegExp("^(?:" + a.config.removeFormatTags.replace(/,/g, "|") + ")$", "i")
						), c = a._.removeAttributes || (
						a._.removeAttributes = a.config.removeFormatAttributes.split(",")
						), h = CKEDITOR.plugins.removeformat.filter, d = a.getSelection().getRanges(1),
					...
					if (j) {
						...
						for (g = g.getNextSourceNode(true, CKEDITOR.NODE_ELEMENT); g;) {
							...
							if (!(
								g.getName() == "img" && g.data("cke-realelement")
								) &&
			...
			CKEDITOR.plugins.add("resize", {init: function (a) {
				...
				i.resize_maxWidth == void 0 && (
					i.resize_maxWidth = 3E3
					);
				i.resize_maxHeight == void 0 && (
					i.resize_maxHeight = 3E3
					);
				i.resize_minWidth == void 0 && (
					i.resize_minWidth = 750
					);
				i.resize_minHeight == void 0 && (
					i.resize_minHeight = 250
					);
				if (i.resize_enabled !== false) {
					var e = null, g = (
						i.resize_dir ==
							"both" || i.resize_dir == "horizontal"
						) && i.resize_minWidth != i.resize_maxWidth, j = (
						i.resize_dir == "both" || i.resize_dir == "vertical"
						) && i.resize_minHeight != i.resize_maxHeight, k = function (e) {
						var k = b, m = c, p = k + (
							e.data.$.screenX - h
							) * (
							f == "rtl" ? -1 : 1
						...
						g && (
							k = Math.max(i.resize_minWidth, Math.min(p, i.resize_maxWidth))
							);
						j && (
							m = Math.max(i.resize_minHeight, Math.min(e, i.resize_maxHeight))
							);
						a.resize(g ? k : null, m)
					}, q = function () {
			...
			(
				function () {
					...
					var b = "", c = function () {
						var a = this, c = function () {
							...
							e.afterMarkupRemove.push(function (a) {
								(
									new CKEDITOR.dom.element(a, e.document)
									).mergeSiblings()
							});
						...
						a.on("contentDomUnload", function () {
							for (var a = CKEDITOR.document.getElementsByTag("script"),
								...
								f && (
									g && f.match(b) && g.match(c)
									) && e.remove()
							}
						});
						a.on("beforeCommandExec", function (b) {
							if ((
								b.data.name == "source" || b.data.name == "newpage"
								) && a.mode == "wysiwyg") {
								if (b = h.getScayt(a)) {
						...
						a.on("afterCommandExec", function (b) {
							h.isScaytEnabled(a) && a.mode == "wysiwyg" && (
								b.data.name == "undo" || b.data.name == "redo"
								) && window.setTimeout(function () {
						...
						(
							d = d && d.htmlFilter
							) && d.addRules({elements: {span: function (a) {
					...
					CKEDITOR.plugins.scayt = {engineLoaded: false, instances: {}, controlInfo: {}, setControlInfo: function (a, b) {
						a && (
							a.name && typeof this.controlInfo[a.name] != "object"
							) && (
					...
					CKEDITOR.plugins.add("scayt", {requires: "menubutton,dialog", beforeInit: function (a) {
						...
						if ((
							b = b.split("|")
							) && b.length)for (var d = 0; d < b.length; d++)c = c + (
							"scayt_" + b[d] + (
								b.length != parseInt(d, 10) + 1 ? "," : ""
								)
							);
			...
			(
				function () {
					...
					CKEDITOR.plugins.add("showborders", {modes: {wysiwyg: 1}, onLoad: function () {
						var a;
						a = (
							CKEDITOR.env.ie6Compat ? [".%1 table.%2,", ".%1 table.%2 td, .%1 table.%2 th", "{", "border : #d3d3d3 1px dotted",
						...
						b.on("removeFormatCleanup", function (a) {
							a = a.data;
							b.getCommand("showborders").state == CKEDITOR.TRISTATE_ON && (
								a.is("table") && (
									!a.hasAttribute("border") || parseInt(a.getAttribute("border"), 10) <= 0
									)
						...
						c && c.addRules({elements: {table: function (a) {
							...
							b && (
								a["class"] = b.replace("cke_show_border", "").replace(/\s{2}/, " ").replace(/^\s+|\s+$/, "")
								)
						}}})
					}});
					CKEDITOR.on("dialogDefinition", function (a) {
							...
							b.getStylesSet(function (d) {
								if (!f.length) {
									for (var e, q, l = 0, n = d.length; l < n; l++) {
										e = d[l];
										if (!(
											b.blockless && e.element in CKEDITOR.dtd.$block
											)) {
											q = e.name;
				...
				function () {
					function a(a) {
						return{editorFocus: false, canUndo: false, modes: {wysiwyg: 1}, exec: function (b) {
							if (b.editable().hasFocus) {
								...
								if (e = (
									new CKEDITOR.dom.elementPath(c.getCommonAncestor(), c.root)
									...
									var c = b.createRange(), g = CKEDITOR.tools.tryThese(function () {
										...
										var b =
											e.getParent(), b = b.getAscendant("table").$.rows[b.$.rowIndex + (
											a ? -1 : 1
											)];
									...
									if (!g && !a) {
										for (var h = e.getAscendant("table").$, g = e.getParent().$.cells, h = new CKEDITOR.dom.element(h.insertRow(-1), b.document), k = 0, q = g.length; k < q; k++) {
											var l = h.append((
												new CKEDITOR.dom.element(g[k], b.document)
												).clone(false, false));
					...
					CKEDITOR.plugins.add("tab", {init: function (d) {
						...
						if (i)d.on("key", function (a) {
							(
								a.data.keyCode ==
									9 && d.execCommand("selectNextCell") || a.data.keyCode == CKEDITOR.SHIFT + 9 && d.execCommand("selectPreviousCell")
								) && a.cancel()
						});
			...
			CKEDITOR.dom.element.prototype.focusNext = function (a, b) {
				...
				if (c <= 0)for (e = this.getNextSourceNode(a, CKEDITOR.NODE_ELEMENT); e;) {
					...
					if (!h)if (!d && e.equals(this)) {
						...
						if (a) {
							if (!(
								e = e.getNextSourceNode(true, CKEDITOR.NODE_ELEMENT)
								))break;
							h = 1
						}
					}
					else d && !this.contains(e) && (
							h = 1
							);
					if (e.isVisible() && !(
						(
			...
			CKEDITOR.dom.element.prototype.focusPrevious = function (a, b) {
				for (var c = b === void 0 ? this.getTabIndex() : b, h, d, i, f = 0, e, g = this.getDocument().getBody().getLast(); g = g.getPreviousSourceNode(false, CKEDITOR.NODE_ELEMENT);) {
					if (!h)if (!d && g.equals(this)) {
						...
						if (a) {
							if (!(
								g = g.getPreviousSourceNode(true, CKEDITOR.NODE_ELEMENT)
								))break;
							h = 1
						}
					}
					else d && !this.contains(g) && (
							h = 1
							);
					if (g.isVisible() && !(
						(
				...
				init: function (a) {
					...
					if (!a.blockless) {
						...
						a.addCommand("tableDelete", b({exec: function (a) {
							...
							if (b) {
								var c = b.getParent();
								c.getChildCount() == 1 && !c.is("body", "td", "th") && (
									b = c
									);
								a = a.createRange();
				...
				function () {
					function a(a) {
						function b(a) {
							if (!(
								c.length > 0
								) && a.type == CKEDITOR.NODE_ELEMENT && l.test(a.getName()) && !a.getCustomData("selected_cell")) {
						...
						for (var a = a.getRanges(),
							...
							if (f.collapsed) {
								...
								(
									f = f.getAscendant("td", true) || f.getAscendant("th", true)
									) && c.push(f)
							}
							else {
								var f = new CKEDITOR.dom.walker(f), g;
								for (f.guard = b; g = f.next();)if ((
									g = g.getAscendant("td") || g.getAscendant("th")
					...
					function c(b) {
						if (b instanceof CKEDITOR.dom.selection) {
									...
									if (l.$.rowSpan == 1)l.remove();
										...
										if (q == h) {
											...
											q[k - 1] ? l.insertAfter(new CKEDITOR.dom.element(q[k -
												1])) : (
												new CKEDITOR.dom.element(e.$.rows[h + 1])
												).append(l, 1)
										}
									}
									k = k + (
							...
							f = e.$.rows;
							e = new CKEDITOR.dom.element(f[g + 1] || (
								b > 0 ? f[b - 1] : null
								) || e.$.parentNode);
					...
					function j(b, c, d) {
						var e = a(b), f;
						if ((
							c ? e.length != 1 : e.length < 2
							) || (
							f = b.getCommonAncestor()
							) && f.type == CKEDITOR.NODE_ELEMENT && f.is("table"))return false;
						var h, b = e[0];
						...
						for (var c = b.getDocument(), J = l, z = y = 0, B = !d && new CKEDITOR.dom.documentFragment(c), F = 0, c = 0; c < e.length; c++) {
							...
							if (!d) {
								...
								if (h.getChildren().count()) {
									if (E != J && A && (
										!A.isBlockBoundary || !A.isBlockBoundary({br: 1})
										))(
										J = B.getLast(CKEDITOR.dom.walker.whitespaces(true))
										) && (
										!J.is || !J.is("br")
										) && B.append("br");
									h.moveChildren(B)
					...
					CKEDITOR.plugins.tabletools = {requires: "table,dialog,contextmenu", init: function (g) {
						...
						g.addCommand("columnDelete", h({exec: function (b) {
							...
							g = new CKEDITOR.dom.element(h[g] || (
								g ? h[g - 1] : b.$.parentNode
								));
							i.length == k && b.remove();
							g && e(g, true)
						}}));
						...
						g.contextMenu && g.contextMenu.addListener(function (a, b, c) {
							return(
								a = c.contains({td: 1, th: 1}, 1)
								) && !a.isReadOnly() ? {tablecell: CKEDITOR.TRISTATE_OFF, tablerow: CKEDITOR.TRISTATE_OFF, tablecolumn: CKEDITOR.TRISTATE_OFF} : null
						})
					}, getSelectedCells: a};
			...
			(
				function () {
							...
							switch (c) {
								...
								case CKEDITOR.SHIFT + 9:
									for (; !h || !h.items.length;) {
										h = c == 9 ? (
											h ? h.next : a.toolbar.next
											) || b.toolbox.toolbars[0] : (
											h ? h.previous : a.toolbar.previous
											) || b.toolbox.toolbars[b.toolbox.toolbars.length - 1];
										if (h.items.length)for (a = h.items[i ? h.items.length - 1 : 0]; a && !a.focus;)(
											a =
												i ? a.previous : a.next
											) || (
											h = 0
								...
								case 38:
									...
									do {
										h = h.previous;
										!h && q && (
											h = a.toolbar.items[a.toolbar.items.length - 1]
											)
									}
									while (h && !h.focus);
									if (h)h.focus();
						...
						b.on("uiSpace", function (e) {
							if (e.data.space == b.config.toolbarLocation) {
											...
											for (var u = 0; u < t.length; u++) {
												...
												if (v = b.ui.create(t[u]))if (v.type == CKEDITOR.UI_SEPARATOR)q = i && v;
													...
													if (!p) {
														...
														p = {id: m, items: []};
														s = r.name && (
															b.lang.toolbar.toolbarGroups[r.name] || r.name
															);
								...
								if (b.config.toolbarCanCollapse && b.elementMode != CKEDITOR.ELEMENT_MODE_INLINE) {
									...
									h.push('<a title="' + (
										g ? b.lang.toolbar.toolbarCollapse : b.lang.toolbar.toolbarExpand
										) + '" id="' + b.ui.spaceId("toolbar_collapser") +
					...
					CKEDITOR.ui.prototype.addToolbarGroup = function (a, c, f) {
						...
						c && (
							c = CKEDITOR.tools.indexOf(e, function (a) {
								return a.name == c
							})
							);
			...
			(
				function () {
					...
					var b = CKEDITOR.plugins.undo.Image = function (a) {
						...
						CKEDITOR.env.ie && b && (
							b = b.replace(/\s+data-cke-expando=".*?"/g,
								"")
							);
						this.contents = b;
			...
			(
				function () {
					function a(a) {
						...
						(
							a = d.getElementById("cke_actscrpt")
							) && a.parentNode.removeChild(a);
					...
					CKEDITOR.plugins.add("wysiwygarea", {init: function (a) {
						a.addMode("wysiwyg", function (b) {
							...
							h.append(d);
							var l = "document.open();" + (
								i ? 'document.domain="' + document.domain + '";' : ""
								) + "document.close();", l = CKEDITOR.env.air ? "javascript:void(0)" : CKEDITOR.env.ie ? "javascript:void(function(){" + encodeURIComponent(l) + "}())" : "", n = CKEDITOR.env.ie || CKEDITOR.env.gecko;
							if (n)d.on("load", c);
					...
					var d = /(^|<body\b[^>]*>)\s*<(p|div|address|h\d|center|pre)[^>]*>\s*(?:<br[^>]*>|&nbsp;|\u00A0|&#160;)?\s*(:?<\/\2>)?\s*(?=$|<\/body>)/gi, i = CKEDITOR.env.isCustomDomain(), f = CKEDITOR.tools.createClass({$: function (b) {
						...
						base: CKEDITOR.editable, proto: {setData: function (a, b) {
							...
							if (b)this.setHtml(a);
								...
								f || (
									n = n + CKEDITOR.tools.buildStyleHtml(c.config.contentsCss)
									);
								...
								else a = d.docType + '<html dir="' + d.contentsLangDirection + '" lang="' + (
									d.contentsLanguage || c.langCode
									) + '"><head><title>' + this._.docTitle + "</title>" + o + n + "</head><body" +
									(
										d.bodyId ? ' id="' + d.bodyId + '"' : ""
										) + (
									d.bodyClass ? ' class="' + d.bodyClass + '"' : ""
									) + ">" + a + "</body></html>";
								if (CKEDITOR.env.gecko) {
									a = a.replace(/<body/, '<body contenteditable="true" ');
									a = a.replace(/<br \/>(?=\s*<\/(:?html|body)>)/, '$&<br type="_moz" />');
									CKEDITOR.env.version < 2E4 && (
										a = a.replace(/<body[^>]*>/, "$&<\!-- cke-content-start --\>")
										)
								}
								a = a.replace(/(?=\s*<\/(:?head)>)/, '<script id="cke_actscrpt" type="text/javascript"' + (
									CKEDITOR.env.ie ? ' defer="defer" ' : ""
									) + ">" + (
									i ? 'document.domain="' + document.domain +
										'";' : ""
									) + "var wasLoaded=0;function onload(){if(!wasLoaded)window.parent.CKEDITOR.tools.callFunction(" + this._.frameLoadedHandler + ",window);wasLoaded=1;}" + (
							...
							b.ignoreEmptyParagraph && (
								c = c.replace(d, function (a, b) {
									return b
								})
								);
							h && (
					...
					CKEDITOR.env.gecko && function () {
						...
						if (a) {
							...
							a.setAttribute("onpageshow", (
								b ? b + ";" : ""
								) + 'event.persisted&&(function(){var x=CKEDITOR.instances,d,i;for(i in x){d=x[i].document;if(d){d.$.designMode="off";d.$.designMode="on";}}})();')
						}
