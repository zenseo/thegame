		...
		jQuery(function () {
			$.tags = function (element, options) {
				var tagData,
					_this = this;
				this.suggestions = (
					options.suggestions != null ? options.suggestions : []
					);
				this.restrictTo = (
					options.restrictTo != null ? options.restrictTo.concat(this.suggestions) : false
					);
				this.exclude = (
					options.excludeList != null ? options.excludeList : false
					);
				this.displayPopovers = (
					options.popovers != null ? true : options.popoverData != null
					);
				this.tagClass = (
					options.tagClass != null ? options.tagClass : 'btn-info'
					);
				this.promptText = (
					options.promptText != null ? options.promptText : 'Enter tags...'
					);
				this.whenAddingTag = (
					options.whenAddingTag != null ? options.whenAddingTag : function (tag) {
					}
					);
				this.definePopover = (
					options.definePopover ? options.definePopover : function (tag) {
						return "associated content for \"" + tag + "\"";
					}
					);
				this.excludes = (
					options.excludes ? options.excludes : function () {
						return false;
					}
					);
				this.tagRemoved = (
					options.tagRemoved ? options.tagRemoved : function (tag) {
					}
					);
				this.pressedReturn = (
					options.pressedReturn != null ? options.pressedReturn : function (e) {
					}
					);
				this.pressedDelete = (
					options.pressedDelete != null ? options.pressedDelete : function (e) {
					}
					);
				this.pressedDown = (
					options.pressedDown != null ? options.pressedDown : function (e) {
					}
					);
				this.pressedUp = (
					options.pressedUp != null ? options.pressedUp : function (e) {
					}
					);
				...
				this.addTag = function (tag) {
					...
					if ((
						_this.restrictTo === false || _this.restrictTo.indexOf(tag) !== -1
						) && _this.tagsArray.indexOf(tag) < 0 && tag.length > 0 && (
						_this.exclude === false || _this.exclude.indexOf(tag) === -1
						) && !_this.excludes(tag)) {
				...
				this.keyDownHandler = function (e) {
					...
					switch (k) {
						...
						case 40:
							...
							if (_this.input.val() === '' && (
								_this.suggestedIndex === -1 || !(
									_this.suggestedIndex != null
									)
								)) {
								_this.makeSuggestions(e, true);
							}
							numSuggestions = _this.suggestionList.length;
							_this.suggestedIndex = (
								_this.suggestedIndex < numSuggestions - 1 ? _this.suggestedIndex + 1 : numSuggestions - 1
								);
						...
						case 38:
							_this.pressedUp(e);
							_this.suggestedIndex = (
								_this.suggestedIndex > 0 ? _this.suggestedIndex - 1 : 0
								);
							_this.selectSuggested(_this.suggestedIndex);
							if (_this.suggestedIndex >= 0) {
								return _this.scrollSuggested(_this.suggestedIndex);
				...
				this.makeSuggestions = function (e, overrideLengthCheck) {
					var val;
					val = (
						e.target.value != null ? e.target.value : e.target.textContent
						);
					_this.suggestedIndex = -1;
					_this.$suggestionList.html('');
					_this.suggestionList = [];
					$.each(_this.suggestions, function (i, suggestion) {
						if (_this.tagsArray.indexOf(suggestion) < 0 && suggestion.substring(0, val.length) === val && (
							val.length > 0 || overrideLengthCheck
							)) {
				...
				this.renderTags = function () {
					...
					tagList.html('');
					_this.input.attr('placeholder', (
						_this.tagsArray.length === 0 ? _this.promptText : ''
						));
			...
			return $.fn.tags = function (options) {
				...
				tagsObject = {};
				stopOn = (
					typeof options === "number" ? options : -1
					);
