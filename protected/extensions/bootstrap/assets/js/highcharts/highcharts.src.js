...
(
	function () {
		 ...
		 */
			TRACKER_FILL = 'rgba(192,192,192,' + (
				hasSVG ? 0.000001 : 0.002
		...
		function pad(number, length) {
			...
			return new Array((
				length || 2
				) + 1 - String(number).length).join(0) + number;
		}
		...
		dateFormat = function (format, timestamp, capitalize) {
			...
			// list all format keys
				replacements = {
					...
					'l': (
						hours % 12
						) || 12, // Hours in 12h format, 1 through 12
		...
		SVGElement.prototype = {
			...
			attr: function (hash, val) {
				...
				if (isWebKit && /Chrome\/(18|19)/.test(userAgent)) {
					if (nodeName === 'text' && (
						hash.x !== UNDEFINED || hash.y !== UNDEFINED
						)) {
			...
			crisp: function (strokeWidth, x, y, width, height) {
				...
				strokeWidth = strokeWidth || wrapper.strokeWidth || (
			...
			htmlUpdateTransform: function () {
				...
				if (elem.tagName === 'SPAN') {
					...
					if (currentTextTransform !== wrapper.cTT) { // do the calculations and DOM access only if properties changed
						...
						xCorr += sintheta * baseline * (
							quad ? 1 - alignCorrection : alignCorrection
							);
						yCorr -= costheta * baseline * (
							rotation ? (
								quad ? alignCorrection : 1 - alignCorrection
								) : 1
							);
		...
		SVGRenderer.prototype = {
			...
			init: function (container, width, height, forExport) {
				...
				if (isFirefox && container.getBoundingClientRect) {
					renderer.subPixelFix = subPixelFix = function () {
						...
						css(container, {
							left: (
								mathCeil(rect.left) - rect.left
								) + PX,
							top: (
								mathCeil(rect.top) - rect.top
								) + PX
						});
					};
			...
			buildText: function (wrapper) {
				...
				function getLineHeightByBBox(lineNo) {
					...
					return mathRound(linePositions[lineNo] - (
						linePositions[lineNo - 1] || 0
						));
				}
			...
			crispLine: function (points, width) {
				...
				if (points[1] === points[4]) {
					points[1] = points[4] = mathRound(points[1]) + (
						width % 2 / 2
						);
				}
				if (points[2] === points[5]) {
					points[2] = points[5] = mathRound(points[2]) + (
						width % 2 / 2
						);
				}
				return points;
				...
				if (gradName) {
					...
					if (!gradAttr.id || !gradients[gradAttr.id]) {
						...
						if (gradName === 'radialGradient' && radialReference && !defined(gradAttr.gradientUnits)) {
							extend(gradAttr, {
								cx: (
									radialReference[0] - radialReference[2] / 2
									) + gradAttr.cx * radialReference[2],
								cy: (
									radialReference[1] - radialReference[2] / 2
			...
			label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
				...
				function updateTextPadding() {
					...
					if (defined(width) && (
						...
						x += { center: 0.5, right: 1 }[textAlign] * (
							width - bBox.width
							);
					}
		...
		Tick.prototype = {
			...
			addLabel: function () {
				var tick = this,
					...
					width = (
						categories && horiz && categories.length && !labelOptions.step && !labelOptions.staggerLines && !labelOptions.rotation &&
			...
			getLabelSize: function () {
				...
				return label ?
					(
						(
							this.labelBBox = label.getBBox(true)
							)
		...
		function StackItem(axis, options, isNegative, x, stackOption) {
			...
			this.alignOptions = {
				...
				y: pick(options.y, inverted ? 4 : (
					isNegative ? 14 : -6
					)),
				x: pick(options.x, inverted ? (
					isNegative ? -6 : 6
					) : 0)
			};
		...
		Axis.prototype = {
			...
			translate: function (val, backwards, cvsCoord, old, handleLog) {
				var axis = this,
					...
					returnValue,
					postTranslate = axis.options.ordinal || (
	 ...
	 */
			...
			setTickPositions: function (secondPass) {
				...
				if (!tickPositions) {
					if (isDatetimeAxis) {
						tickPositions = (
							axis.getNonLinearTimeTicks || getTimeTicks
							)(
		...
		Tooltip.prototype = {
			...
			move: function (finalX, finalY) {
				...
				tooltip.currentX = tooltip.tooltipIsHidden ? finalX : (
					2 * tooltip.currentX + finalX
					) / 3;
				tooltip.currentY = tooltip.tooltipIsHidden ? finalY : (
			...
			getPosition: function (boxWidth, boxHeight, point) {
				...
				if (y < plotTop + 5) {
					...
					if (alignedRight && pointY >= y && pointY <= (
						y + boxHeight
						)) {
			...
			refresh: function (point, mouseEvent) {
				...
				if (shared && !(
					point.series && point.series.noSharedTooltip
					)) {
		...
		MouseTracker.prototype = {
			...
			onmousemove: function (e) {
				...
				// shared tooltip
				if (chart.tooltip && mouseTracker.options.tooltip.shared && !(
					...
					if (points.length && (
						points[0].plotX !== mouseTracker.hoverX
						)) {
			...
			setDOMEvents: function () {
				...
				container.onclick = function (e) {
					...
					if (!chart.cancelClick) {
						...
						if (hoverPoint && (
							attr(e.target, 'isTracker') || attr(e.target.parentNode, 'isTracker')
							)) {
				...
				each(legend.allItems, function (item) {
					...
					if (checkbox) {
						css(checkbox, {
							...
							top: (
								alignAttr.translateY + checkbox.y + 3
								) + PX
						});
					}
				});
			...
			scroll: function (scrollBy, animation) {
				...
				if (currentPage > 0) {
					...
					this.scrollGroup.animate({
						translateY: -mathMin(clipHeight * (
							currentPage - 1
							), this.fullHeight - clipHeight + padding) + 1
					});
					pager.attr({
		...
		Chart.prototype = {
			...
			getMargins: function () {
				...
				if ((
					...
					titleOffset = mathMax(
						(
							chart.title && !chartTitleOptions.floating && !chartTitleOptions.verticalAlign && chartTitleOptions.y
							) || 0,
						(
							chart.subtitle && !chartSubtitleOptions.floating && !chartSubtitleOptions.verticalAlign && chartSubtitleOptions.y
							) || 0
					);
					if (titleOffset) {
			...
			resize: function (width, height, animation) {
				...
				if (globalAnimation === false) {
					...
					setTimeout(chart.fireEndResize, (
						globalAnimation && globalAnimation.duration
						) || 500);
				}
			},
			...
			drawChartBox: function () {
				...
				// Chart area
				mgn = chartBorderWidth + (
					optionsChart.shadow ? 8 : 0
					);
			...
			firstRender: function () {
				...
				if ((
					!hasSVG && (
						win == win.top && doc.readyState !== COMPLETE
						)
					) || (
					useCanVG && !win.canvg
					)) {
			...
			tooltipFormatter: function (pointFormat) {
				...
				for (i in match) {
					...
					if (isString(key) && key !== pointFormat) { // IE matches more than just the variables
						...
						parts = (
							' ' + key
							).split(splitter); // add empty string because IE and the rest handles it differently
			...
			setTooltipPoints: function (renew) {
				...
				for (i = 0; i < pointsLength; i++) {
					...
					point._high = high = points[i + 1] ?
						mathMax(0, mathFloor((
							point.plotX + (
								points[i + 1] ? points[i + 1].plotX : plotSize
								)
							) / 2)) :
						plotSize;
			...
			drawDataLabels: function () {
				...
				if (options.enabled || series._hasPointLabels) {
					...
					each(points, function (point) {
						...
						if (enabled) {
							var plotX = (
									point.barX && point.barX + point.barW / 2
									) || pick(point.plotX, -999),
						...
						if (dataLabel && series.isCartesian && (
							!chart.isInsidePlot(x, y) || !enabled
							)) {
			...
			render: function () {
				...
				setTimeout(function () {
					...
					if (group && clipRect !== chart.clipRect && clipRect.renderer) {
						if (doClip) {
							group.clip((
								series.clipRect = chart.clipRect
								));
						}
						clipRect.destroy();
					}
		...
		var SplineSeries = extendClass(Series, {
			...
			getPointSpline: function (segment, point, i) {
				...
				if (i && i < segment.length - 1) {
					...
					leftContX = (
						smoothing * plotX + lastX
						) / denom;
					leftContY = (
						smoothing * plotY + lastY
						) / denom;
					rightContX = (
						smoothing * plotX + nextX
						) / denom;
					rightContY = (
						smoothing * plotY + nextY
						) / denom;
		...
		var ColumnSeries = extendClass(Series, {
			...
			translate: function () {
				...
				var points = series.points,
					categoryWidth = mathAbs(xAxis.transA) * (
					...
					pointPadding = defined(optionPointWidth) ? (
						pointOffsetWidth - optionPointWidth
						) / 2 :
						pointOffsetWidth * options.pointPadding,
					pointWidth = pick(optionPointWidth, pointOffsetWidth - 2 * pointPadding), // exact point width, used in polar charts
					barW = mathCeil(mathMax(pointWidth, 1 + 2 * borderWidth)), // rounded and postprocessed for border width
					colIndex = (
						reversedXAxis ? columnCount -
							series.columnIndex : series.columnIndex
						) || 0,
					pointXOffset = pointPadding + (
						groupPadding + colIndex *
							pointOffsetWidth - (
							categoryWidth / 2
							)
						) *
		...
		var PiePoint = extendClass(Point, {
			...
			slice: function (sliced, redraw, animation) {
				...
				translation = {
					translateX: (
						sliced ? slicedTranslation[0] : chart.plotLeft
						),
					translateY: (
		...
		var PieSeries = {
			...
			drawDataLabels: function () {
				...
				labelHeight = halves[0][0] && halves[0][0].dataLabel && (
					halves[0][0].dataLabel.getBBox().height || 21
					); // 21 is for #968
