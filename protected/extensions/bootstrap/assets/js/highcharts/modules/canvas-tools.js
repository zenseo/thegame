		...
		function m() {
			...
			a.Property = function (c, d) {
				...
				this.Angle = {toRadians: function () {
					...
					var a = b.value +
						"";
					return a.match(/deg$/) ? b.numValue() * (
						Math.PI / 180
						) : a.match(/grad$/) ? b.numValue() * (
						Math.PI / 200
						) : a.match(/rad$/) ? b.numValue() : b.numValue() * (
						Math.PI / 180
						)
				}}
			};
			...
			a.ToNumberArray = function (c) {
				for (var c = a.trim(a.compressSpaces((
					c || ""
					).replace(/,/g, " "))).split(" "), d = 0; d < c.length; d++) {
					c[d] = parseFloat(c[d]);
				}
				return c
			};
			...
			a.BoundingBox = function (a, d, b, k) {
				...
				this.addQuadraticCurve = function (b, a, d, c, k, l) {
					...
					this.addBezierCurve(b, a, d, d + 1 / 3 * (
						k - b
						), c, c + 1 / 3 * (
						l - a
						), k, l)
				...
				this.addBezierCurve = function (b, a, d, c, k, l, o, n) {
					...
					for (i = 0; i <= 1; i++) {
						b = function (b) {
							return Math.pow(1 - b, 3) * q[i] + 3 * Math.pow(1 - b, 2) * b * p[i] + 3 * (
								1 - b
								) * Math.pow(b, 2) * t[i] + Math.pow(b, 3) * m[i]
						}, a = 6 * q[i] - 12 * p[i] + 6 * t[i], d = -3 * q[i] + 9 * p[i] - 9 * t[i] + 3 * m[i], c = 3 * p[i] - 3 * q[i], d == 0 ? a != 0 && (
							a = -c / a, 0 < a && a < 1 && (
			...
			a.AspectRatio = function (c, d, b, k, e, f, g, j, h, l) {
				...
				h.hasValue() && l.hasValue() ? c.translate(-p * h.Length.toPixels("x"), -p * l.Length.toPixels("y")) : (
					o.match(/^xMid/) && (
			...
			a.Element.ElementBase = function (c) {
				...
				if (c != null && c.nodeType == 1) {
					...
					this.attribute("id").hasValue() && a.Definitions[this.attribute("id").value] == null && (
			...
			a.Element.RenderedElementBase = function (c) {
				...
				this.setContext = function (d) {
					...
					this.attribute("transform").hasValue() && (
						new a.Transform(this.attribute("transform").value)
						).apply(d);
			...
			a.Element.rect = function (c) {
				...
				this.path = function (d) {
					...
					d != null && (
						d.beginPath(), d.moveTo(b + g, c), d.lineTo(b + e - g, c), d.quadraticCurveTo(b + e, c, b + e, c + j), d.lineTo(b + e, c + f - j), d.quadraticCurveTo(b + e, c + f, b + e - g, c + f), d.lineTo(b + g, c + f), d.quadraticCurveTo(b, c + f, b, c + f - j), d.lineTo(b, c + j), d.quadraticCurveTo(b, c, b + g, c), d.closePath()
						);
			...
			a.Element.ellipse = function (c) {
				...
				this.path = function (d) {
					var b = 4 * (
						...
						), c = this.attribute("rx").Length.toPixels("x"), e = this.attribute("ry").Length.toPixels("y"), f = this.attribute("cx").Length.toPixels("x"), g = this.attribute("cy").Length.toPixels("y");
					d != null && (
						d.beginPath(), d.moveTo(f, g - e), d.bezierCurveTo(f + b * c, g - e, f + c, g - b * e, f + c, g), d.bezierCurveTo(f + c, g + b * e, f + b * c, g + e, f, g + e), d.bezierCurveTo(f - b * c, g + e, f - c, g + b * e, f - c, g), d.bezierCurveTo(f - c, g - b * e, f - b * c, g - e, f, g - e), d.closePath()
						);
				...
				this.path = function (d) {
					...
					for (d != null && d.beginPath(); !b.isEnd();) {
						switch (b.nextCommand(), b.command.toUpperCase()) {
							...
							case "H":
								for (; !b.isCommandOrEnd();) {
									e = new a.Point((
										...
										) + b.getScalar(), b.current.y), b.addMarker(e, b.current), b.current = e, c.addPoint(b.current.x, b.current.y), d != null && d.lineTo(b.current.x, b.current.y);
								}
								break;
							case "V":
								for (; !b.isCommandOrEnd();) {
									e = new a.Point(b.current.x, (
										b.isRelativeCommand() ? b.current.y : 0
										) + b.getScalar()), b.addMarker(e, b.current), b.current = e, c.addPoint(b.current.x, b.current.y), d != null && d.lineTo(b.current.x, b.current.y);
								}
								break;
							case "C":
								for (; !b.isCommandOrEnd();) {
							...
							case "A":
								for (; !b.isCommandOrEnd();) {
									var g = b.current, h = b.getScalar(), l = b.getScalar(), f = b.getScalar() * (
											Math.PI / 180
											), o = b.getScalar(), j = b.getScalar(),
										e = b.getAsCurrentPoint(), n = new a.Point(Math.cos(f) * (
											g.x - e.x
											) / 2 + Math.sin(f) * (
											g.y - e.y
											) / 2, -Math.sin(f) * (
											...
											g.y - e.y
											) / 2), q = Math.pow(n.x, 2) / Math.pow(h, 2) + Math.pow(n.y, 2) / Math.pow(l, 2);
									q > 1 && (
									...
									var p = new a.Point(o * h * n.y / l, o * -l * n.x / h), g = new a.Point((
										g.x + e.x
										) / 2 + Math.cos(f) *
										p.x - Math.sin(f) * p.y, (
										g.y + e.y
										) / 2 + Math.sin(f) * p.x + Math.cos(f) * p.y), m = function (b, a) {
										return(
											b[0] * a[0] + b[1] * a[1]
											) / (
											Math.sqrt(Math.pow(b[0], 2) + Math.pow(b[1], 2)) * Math.sqrt(Math.pow(a[0], 2) + Math.pow(a[1], 2))
											)
									}, s = function (b, a) {
										return(
											b[0] * a[1] < b[1] * a[0] ? -1 : 1
			...
			a.Element.radialGradient = function (c) {
				...
				this.getGradient = function (a, b) {
					...
					this.attribute("fy").hasValue() && (
						j = this.gradientUnits == "objectBoundingBox" ? c.y() + c.height() * this.attribute("fy").numValue() :
							this.attribute("fy").Length.toPixels("y")
						);
					c = this.gradientUnits == "objectBoundingBox" ? (
						c.width() + c.height()
						) / 2 * this.attribute("r").numValue() : this.attribute("r").Length.toPixels();
			...
			a.Element.font = function (c) {
				...
				for (c = 0; c < this.children.length; c++) {
					...
					if (d.type == "font-face") {
						...
						d.arabicForm != "" ? (
							this.isArabic = this.isRTL = !0, typeof this.glyphs[d.unicode] == "undefined" && (
				...
				this.renderChildren =
					function (b) {
						for (var a = this.style("text-anchor").valueOrDefault("start"), c = this.attribute("x").Length.toPixels("x"), d = this.attribute("y").Length.toPixels("y"), j = 0; j < this.children.length; j++) {
							...
							h.attribute("x").hasValue() ? h.x = h.attribute("x").Length.toPixels("x") : (
							...
							c = h.x + c;
							h.attribute("y").hasValue() ? h.y = h.attribute("y").Length.toPixels("y") : (
				...
				this.getGlyph = function (a, b, c) {
					...
					if (a.isArabic) {
						...
						typeof a.glyphs[e] != "undefined" && (
							f = a.glyphs[e][g], f == null && a.glyphs[e].type == "glyph" && (
								f = a.glyphs[e]
								)
							)
					}
				...
				this.renderChildren = function (c) {
					...
					if (b != null) {
						...
						for (var g = a.ToNumberArray(this.parent.attribute("dx").value), j = 0; j < f.length; j++) {
							...
							c.translate(-this.x, -this.y);
							this.x += k * (
								h.horizAdvX || b.horizAdvX
								) / b.fontFace.unitsPerEm;
							typeof g[j] != "undefined" && !isNaN(g[j]) && (
								this.x += g[j]
								)
						}
					}
				...
				this.measureText = function (c) {
					...
					if (b != null) {
						...
						for (var f = a.ToNumberArray(this.parent.attribute("dx").value), g = 0; g < e.length; g++) {
							...
							k += (
								j.horizAdvX || b.horizAdvX
								) * c / b.fontFace.unitsPerEm;
							typeof f[g] != "undefined" && !isNaN(f[g]) && (
								k += f[g]
								)
						}
						return k
			...
			a.Element.image = function (c) {
				...
				this.renderChildren = function (b) {
					...
					f == 0 || g == 0 || (
						b.save(), b.translate(c, d), a.AspectRatio(b, this.attribute("preserveAspectRatio").value, f, this.img.width, g, this.img.height, 0, 0), b.drawImage(this.img, 0, 0), b.restore()
						)
				}
			};
			...
			a.Element.style = function (c) {
				this.base = a.Element.ElementBase;
				this.base(c);
				for (var c = c.childNodes[0].nodeValue + (
			...
			a.loadXmlDoc = function (c, d) {
				...
				var k = a.CreateElement(d.documentElement), e = k.root = !0, f = function () {
					...
					k.render(c);
					e && (
						e = !1, a.opts != null && typeof a.opts.renderCallback == "function" && a.opts.renderCallback()
						)
				}, g = !0;
				a.ImagesLoaded() && (
					g = !1, f()
				...
				a.intervalID = setInterval(function () {
					...
					a.opts != null && typeof a.opts.forceRedraw == "function" && a.opts.forceRedraw() == !0 && (
						b = !0
						);
					b && (
						f(), a.Mouse.runEvents()
		...
		this.canvg = function (a, c, d) {
			if (a == null && c == null && d == null) {
				...
				d = d || {}, typeof a == "string" && (
					...
					b = a.svg, b.stop()
					), b.opts = d, a = a.getContext("2d"), typeof c.documentElement != "undefined" ? b.loadXmlDoc(a, c) : c.substr(0, 1) == "<" ? b.loadXml(a, c) : b.load(a, c)
			}
		}
	}
	)();
...
(
	function (m) {
		...
		b(c.prototype, {create: function (a, b, c, d) {
			...
			d.shadow && (
				m = k(m, {"box-shadow": "1px 1px 3px gray",
