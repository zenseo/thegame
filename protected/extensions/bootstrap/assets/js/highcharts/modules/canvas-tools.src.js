	...
	function () {
		...
		function build() {
			...
			svg.ToNumberArray = function (s) {
				var a = svg.trim(svg.compressSpaces((
					s || ''
					).replace(/,/g, ' '))).split(' ');
			...
			svg.Element.svg = function (node) {
				...
				this.setContext = function (ctx) {
					...
					if (typeof(
						this.root
						) == 'undefined' && this.attribute('width').hasValue() && this.attribute('height').hasValue()) {
			...
			svg.Element.ellipse = function (node) {
				...
				this.path = function (ctx) {
					...
					if (ctx != null) {
						ctx.beginPath();
						ctx.moveTo(cx, cy - ry);
						ctx.bezierCurveTo(cx + (
							KAPPA * rx
							), cy - ry, cx + rx, cy - (
							KAPPA * ry
							), cx + rx, cy);
						ctx.bezierCurveTo(cx + rx, cy + (
							KAPPA * ry
							), cx + (
							KAPPA * rx
							), cy + ry, cx, cy + ry);
						...
						switch (pp.command.toUpperCase()) {
							...
							case 'A':
								while (!pp.isCommandOrEnd()) {
									...
									var rx = pp.getScalar();
									var ry = pp.getScalar();
									var xAxisRotation = pp.getScalar() * (
										Math.PI / 180.0
										);
									...
									var centp = new svg.Point(
										(
											curr.x + cp.x
											) / 2.0 + Math.cos(xAxisRotation) * cpp.x - Math.sin(xAxisRotation) * cpp.y,
										(
											curr.y + cp.y
											) / 2.0 + Math.sin(xAxisRotation) * cpp.x + Math.cos(xAxisRotation) * cpp.y
									);
									// vector magnitude
									var m = function (v) {
									...
									pp.addMarkerAngle(halfWay, (
										a1 + ad
										) / 2 + (
										sweepFlag == 0 ? 1 : -1
										) * Math.PI / 2);
									pp.addMarkerAngle(cp, ad + (
										sweepFlag == 0 ? 1 : -1
										) * Math.PI / 2);
				...
				this.renderChildren = function (ctx) {
					...
					for (var i = 0; i < this.children.length; i++) {
						...
						if (textAnchor != 'start' && (
							...
							child.x -= (
								textAnchor == 'end' ? groupLength : groupLength / 2.0
								);
						}
			...
			svg.loadXmlDoc = function (ctx, dom) {
				...
				var draw = function () {
					...
					if (svg.opts != null && svg.opts['scaleWidth'] != null && svg.opts['scaleHeight'] != null) {
						...
						e.attribute('viewBox', true).value = '0 0 ' + (
							cWidth * xRatio
							) + ' ' + (
							cHeight * yRatio
							);
