...
rangy.createModule("DomUtil", function (api, module) {
	...
	function isHtmlNamespace(node) {
		...
		return typeof node.namespaceURI == UNDEF || (
			(
				...
				) === null || ns == "http://www.w3.org/1999/xhtml"
			);
	}
	...
	function comparePoints(nodeA, offsetA, nodeB, offsetB) {
		...
		if (nodeA == nodeB) {
			...
			childA = (
				nodeA === root
				) ? root : getClosestAncestorIn(nodeA, root, true);
			childB = (
				nodeB === root
				) ? root : getClosestAncestorIn(nodeB, root, true);
...
rangy.createModule("DomRange", function (api, module) {
	...
	function inspect(range) {
		var name = (
			typeof range.getName == "undefined"
			) ? "Range" : range.getName();
	...
	try {
		...
		htmlParsingConforms = (
			styleEl.firstChild.nodeType == 3
			); // Opera incorrectly creates an element node
	} catch (e) {
		// IE 6 and 7 throw
	}
...
rangy.createModule("WrappedSelection", function (api, module) {
	...
	if (useDocumentSelection) {
		...
		api.isSelectionValid = function (winParam) {
			var doc = (
				winParam || window
				).document, nativeSel = doc.selection;

			// Check whether the selection TextRange is actually contained within the correct document
			return (
				nativeSel.type != "None" || dom.getDocument(nativeSel.createRange().parentElement()) == doc
				);
		};
	}
	...
	if (!useDocumentSelection && selectionHasAnchorAndFocus && api.features.implementsDomRange) {
		selectionIsBackwards = function (sel) {
			...
			if (sel.anchorNode) {
				backwards = (
					dom.comparePoints(sel.anchorNode, sel.anchorOffset, sel.focusNode, sel.focusOffset) == 1
					);
			}
			return backwards;
		};
	...
	function inspect(sel) {
		...
		var name = (
			typeof sel.getName == "function"
			) ? sel.getName() : "Selection";
...
wysihtml5.browser = (
	function () {
		...
		function iosVersion(userAgent) {
			return (
				(
					/ipad|iphone|ipod/.test(userAgent) && userAgent.match(/ os (\d+).+? like mac os x/)
					) || [, 0]
				)[1];
		}
		...
		return {
			...
			supported: function () {
				...
				// contentEditable is unusable in mobile browsers (tested iOS 4.2.2, Android 2.2, Opera Mobile, WebOS 3.05)
					isIncompatibleMobileBrowser = (
						...
						) || userAgent.indexOf("opera mobi") !== -1 || userAgent.indexOf("hpwos/") !== -1;
			...
			supportsDataTransfer: function () {
				try {
					...
					return isWebKit && (
						window.Clipboard || window.DataTransfer
						).prototype.getData;
				} catch (e) {
					return false;
				}
       ...
       */
		...
		function _convertUrlsToLinks(str) {
			return str.replace(URL_REG_EXP, function (match, url) {
				var punctuation = (
						url.match(TRAILING_CHAR_REG_EXP) || []
						)[1] || "",
		...
		function convertToList(element, listType) {
			...
			for (i = 0; i < lineBreaksLength; i++) {
				...
				while ((
					...
					) && parentNode !== element && parentNode.lastChild === lineBreak) {
				...
				to: function (elementToCopyTo) {
					...
					for (; i < length; i++) {
						...
						if (typeof(
							...
							) !== "undefined" && elementToCopyFrom[attribute] !== "") {
...
(
	function (wysihtml5) {
		 ...
		 *    wysihtml5.quirks.ensureProperClearing(myContentEditableElement);
		 */
		wysihtml5.quirks.ensureProperClearingOfLists = (
			function () {
...
(
	function (wysihtml5, rangy) {
		...
		function splitNodeAt(node, descendantNode, descendantOffset) {
			...
			return (
				descendantNode == node
				) ? newNode : splitNodeAt(node, newNode.parentNode, rangy.dom.getNodeIndex(newNode));
		}

		function Merge(firstNode) {
			this.isElementMerge = (
				firstNode.nodeType == wysihtml5.ELEMENT_NODE
				);
		...
		HTMLApplier.prototype = {
			...
			areElementsMergeable: function (el1, el2) {
				return rangy.dom.arrayContains(this.tagNames, (
					el1.tagName || ""
					).toLowerCase())
	...
	function (wysihtml5) {
		...
		wysihtml5.commands.formatBlock = {
			exec: function (composer, command, nodeName, className, classRegExp) {
				...
				nodeName = typeof(
					nodeName
					) === "string" ? nodeName.toUpperCase() : nodeName;
				...
				if (blockElement) {
					composer.selection.executeAndRestoreSimple(function () {
						...
						if (!hasClasses && blockElement.nodeName === (
			...
			/** @scope wysihtml5.toolbar.Toolbar.prototype */ {
				constructor: function (editor, container) {
					...
					this.container = typeof(
						container
						) === "string" ? document.getElementById(container) : container;
					this.composer = editor.composer;
		...
		wysihtml5.Editor = wysihtml5.lang.Dispatcher.extend(
			/** @scope wysihtml5.Editor.prototype */ {
				constructor: function (textareaElement, config) {
					this.textareaElement = typeof(
						textareaElement
						) === "string" ? document.getElementById(textareaElement) : textareaElement;
