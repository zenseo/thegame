...
Yii::import('ext.bootstrap.widgets.TbJsonDataColumn');
...
class TbJsonGridView extends TbGridView
{
	...
	{
		if ($this->json) {
			echo function_exists('json_encode') ? json_encode($this->renderTableBody())
				: CJSON::encode($this->renderTableBody());
	...
	protected function renderTemplates()
	{
		...
		echo $this->renderTemplate($this->id . '-summary-template', '<div class="<%=this.class%>"><%=this.text%></div>');
	...
	protected function renderTableRowJSON($row)
	{
		...
		if ($this->rowCssClassExpression !== null) {
			$data = $this->dataProvider->data[$row];
			$json['class'] = $this->evaluateExpression($this->rowCssClassExpression, array(
					'row' => $row,
					'data' => $data
				));
		}
