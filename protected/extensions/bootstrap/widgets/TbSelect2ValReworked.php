 ...
 */
class TbSelect2ValReworked extends CInputWidget
{
	/**
	...
	public function run()
	{
		...
		if ($this->hasModel()) {
			...
			echo $this->asDropDownList ? CHtml::dropDownList($name, $this->value, $this->data, $this->htmlOptions)
	...
	public function registerClientScript($id)
	{
		...
		// Небольшая доработка обработки свойства 'val'
		// чтобы можно было подставлять массив (для мультиселекта)
		if (!empty($this->val)) {
			// Если мы передаем в Селект 2 массив, то он энкодится в массив json и скармливается select2
			if (is_array($this->val)) {
				$defValue = ".select2('val', " . CJSON::encode($this->val) . ")";
			}
			else {
				// Если же мы поленились и передали ему строку, то он превратит строку в массив
				// и потом снова энкодит в массив json и скармливает select2
				$defValue = ".select2('val', " . CJSON::encode(explode(',', $this->val)) . ")";
			}
		}
		else {
			$defValue = '';
