...
class Bootstrap extends CApplicationComponent
{
	...
	protected function appendUserSuppliedPackagesToOurs()
	{
		...
		$this->packages = CMap::mergeArray($bootstrapPackages, $this->packages);
	}
	...
	{
		$this->assetsRegistry->registerScript($this->getUniqueScriptId(), '$("' . $selector . '").daterangepicker(' . CJavaScript::encode($options) . ($callback
	...
	public function registerPlugin($name, $selector = null, $options = array())
	{
		...
		$this->assetsRegistry->registerScript($this->getUniqueScriptId(), "jQuery('{$selector}').{$name}({$options});");
	}
