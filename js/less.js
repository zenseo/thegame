...
function errorConsole(a, b) {
	var c = "{line} {content}", d = a.filename || b, e = [], f = (
		a.type || "Syntax"
		) + "Error: " + (
		a.message || "There is an error in your .less file"
		) + " in " + d + " ", g = function (a, b, d) {
		void 0 !== a.extract[b] && e.push(c.replace(/\{line\}/, (
			parseInt(a.line, 10) || 0
			) + (
			b - 1
			)).replace(/\{class\}/, d).replace(/\{content\}/, a.extract[b]))
	};
	a.extract ? (
		g(a, 0, ""), g(a, 1, "line"), g(a, 2, ""), f += "on line " + a.line + ", column " + (
			a.column + 1
			) + ":\n" + e.join("\n")
		) : a.stack && (
		f += a.stack
...
function createCSS(a, b, c) {
	var d = b.href || "", e = "less:" + (
		b.title || extractId(d)
		), f = document.getElementById(e), g = !1, h = document.createElement("style");
...
function errorHTML(a, b) {
	...
	g.id = e, g.className = "less-error-message", d = "<h3>" + (
		...
		a.message || "There is an error in your .less file"
		) + "</h3>" + '<p>in <a href="' + i + '">' + j + "</a> ";
	var k = function (a, b, c) {
		void 0 !== a.extract[b] && h.push(f.replace(/\{line\}/, (
			parseInt(a.line, 10) || 0
			) + (
			b - 1
			)).replace(/\{class\}/, c).replace(/\{content\}/, a.extract[b]))
	};
	a.extract ? (
		k(a, 0, ""), k(a, 1, "line"), k(a, 2, ""), d += "on line " + a.line + ", column " + (
			a.column + 1
			) + ":</p>" + "<ul>" + h.join("") + "</ul>"
		) : a.stack && (
		d += "<br/>" + a.stack.split("\n").slice(1).join("<br/>")
		), g.innerHTML = d, createCSS([".less-error-message ul, .less-error-message li {", "list-style-type: none;", "margin-right: 15px;", "padding: 4px 0;", "margin: 0;", "}", ".less-error-message label {", "font-size: 12px;", "margin-right: 15px;", "padding: 4px 0;", "color: #cc7777;", "}", ".less-error-message pre {", "color: #dd6666;", "padding: 4px 0;", "margin: 0;", "display: inline-block;", "}", ".less-error-message pre.line {", "color: #ff0000;", "}", ".less-error-message h3 {", "font-size: 20px;", "font-weight: bold;", "padding: 15px 0 5px 0;", "margin: 0;", "}", ".less-error-message a {", "color: #10a", "}", ".less-error-message .error {", "color: red;", "font-weight: bold;", "padding-bottom: 2px;", "border-bottom: 1px dashed red;", "}"].join("\n"), {title: "error-message"}), g.style.cssText = ["font-family: Arial, sans-serif", "border: 1px solid #e00", "background-color: #eee", "border-radius: 5px", "-webkit-border-radius: 5px", "-moz-border-radius: 5px", "color: #e00", "padding: 15px", "margin-bottom: 15px"].join(";"), "development" == less.env && (
		c = setInterval(function () {
			document.body && (
				document.getElementById(e) ? document.body.replaceChild(g, document.getElementById(e)) : document.body.insertBefore(g, document.body.firstChild), clearInterval(c)
				)
		}, 10)
		)
}
...
function loadStyles(a) {
	for (var b, c = document.getElementsByTagName("style"), d = 0; d < c.length; d++) {
		if (b = c[d], b.type.match(typePattern)) {
			...
			e.filename = document.location.href.replace(/#.*$/, ""), a && (
				e.useFileCache = !0, f += "\n" + a
				);
			var g = function (a) {
				return function (b, c) {
...
function extractUrlParts(a, b) {
	...
	return g.hostPart = f[1], g.directories = h, g.path = f[1] + h.join("/"), g.fileUrl = g.path + (
		f[4] || ""
		), g.url = g.fileUrl + (
		f[5] || ""
		), g
...
function loadFile(a, b, c, d, e) {
	b && b.currentDirectory && !/^([a-z-]+:)?\//.test(a) && (
		a = b.currentDirectory + a
		);
	var f = extractUrlParts(a, window.location.href), g = f.url, h = {currentDirectory: f.path, filename: g};
	if (b ? (
		h.entryPath = b.entryPath, h.rootpath = b.rootpath, h.rootFilename = b.rootFilename, h.relativeUrls = b.relativeUrls
		) : (
		h.entryPath = f.path, h.rootpath = less.rootpath || f.path, h.rootFilename = g, h.relativeUrls = d.relativeUrls
		), h.relativeUrls && (
		...
		try {
			...
			e && (
				i += "\n" + e
				), c(null, i, g, h, {lastModified: new Date})
		} catch (j) {
			c(j, null, g)
		}
	}
...
function loadStyleSheet(a, b, c, d, e) {
	...
	f.mime = a.type, e && (
		f.useFileCache = !0
		), loadFile(a.href, null, function (e, g, h, i, j) {
		if (j) {
			j.remaining = d;
		...
		removeError(h), g ? (
			f.currentFileInfo = i, new less.Parser(f).parse(g, function (c, d) {
				if (c) {
...
function loadStyleSheets(a, b, c) {
	for (var d = 0; d < less.sheets.length; d++) {
		loadStyleSheet(less.sheets[d], a, b, less.sheets.length - (
			d + 1
			), c)
	}
}
...
(
	"undefined" == typeof window.less || "undefined" != typeof window.less.nodeType
	) && (
	window.less = {}
...
void 0 === less && (
	...
	function g(a) {
		...
		return u[q] = u[q].slice(a + (
			p - f
			)), v = p, 0 === u[q].length && q < u.length - 1 && q++, b !== p || c !== q
	}
	...
	function m(a, b, c) {
		...
		return"browser" !== less.mode && "rhino" !== less.mode && (
	...
	a instanceof tree.parseEnv || (
		a = new tree.parseEnv(a)
		);
	var y = this.imports = {paths: a.paths || [], queue: [], files: a.files, contents: a.contents, mime: a.mime, error: null, push: function (b, c, d, e) {
		...
		less.Parser.importer ? less.Parser.importer(b, c, g, a) : less.Parser.fileLoader(b, c, function (b, e, f, h) {
			...
			i.currentFileInfo = h, i.processImports = !1, i.contents[f] = e, (
	...
	return n.prototype = new Error, n.prototype.constructor = n, this.env = a = a || {}, this.optimization = "optimization"in this.env ? this.env.optimization : 1, w = {imports: y, parse: function (b, c) {
		...
		if (p = q = v = t = 0, o = b.replace(/\r\n/g, "\n"), o = o.replace(/^\uFEFF/, ""), w.imports.contents[a.currentFileInfo.filename] = o, u = function (b) {
			for (var c, d, e, f, g = 0, i = /(?:@\{[\w-]+\}|[^"'`\{\}\/\(\)\\])+/g, j = /\/\*(?:[^*]|\*+[^\/*])*\*+\/|\/\/.*/g, k = /"((?:[^"\\\r\n]|\\.)*)"|'((?:[^'\\\r\n]|\\.)*)'|`((?:[^`]|\\.)*)`/g, l = 0, m = b[0], p = 0; p < o.length;) {
				if (i.lastIndex = p, (
					...
					), e = o.charAt(p), j.lastIndex = k.lastIndex = p, (
					c = k.exec(o)
					) && c.index === p) {
					p += c[0].length, m.push(c[0]);
				}
			...
			return 0 !== l && (
				h = new n({index: p - 1, type: "Parse", message: l > 0 ? "missing closing `}`" : "missing opening `{`", filename: a.currentFileInfo.filename}, a)
				), b.map(function (a) {
				return a.join("")
			})
		}([
			...
			return function (c, d) {
				...
				try {
					e = b.call(this, g), (
						...
						).run(e), new tree.toCSSVisitor({compress: Boolean(c.compress)}).run(e), c.sourceMap && (
						e = new tree.sourceMapOutput({writeSourceMap: c.writeSourceMap, rootNode: e, contentsMap: w.imports.contents, sourceMapFilename: c.sourceMapFilename, outputFilename: c.sourceMapOutputFilename, sourceMapBasepath: c.sourceMapBasepath, sourceMapRootpath: c.sourceMapRootpath, outputSourceFiles: c.outputSourceFiles, sourceMapGenerator: c.sourceMapGenerator})
						), f = e.toCSS({compress: Boolean(c.compress), dumpLineNumbers: a.dumpLineNumbers, strictUnits: Boolean(c.strictUnits)})
				} catch (h) {
					throw new n(h, a)
				}
		...
		var k = function (b) {
			return b = h || b || w.imports.error, b ? (
				b instanceof n || (
					b = new n(b, a)
					), c(b)
				) : c(null, d)
		};
		return a.processImports === !1 ? k() : (
			new tree.importVisitor(this.imports, k).run(d), void 0
			)
	}, parsers: {primary: function () {
		for (var a, b = []; (
			a = f(this.extendRule) || f(this.mixin.definition) || f(this.rule) || f(this.ruleset) || f(this.mixin.call) || f(this.comment) || f(this.directive)
			) || f(/^[\s\n]+/) || f(/^;+/);) {
			a && b.push(a);
		}
		return b
		...
		var b;
		if ("/" === o.charAt(p)) {
			return"/" === o.charAt(p + 1) ? new tree.Comment(f(/^\/\/.*/), !0, p, a.currentFileInfo) : (
				b = f(/^\/\*(?:[^*]|\*+[^\/*])*\*+\/\n?/)
		...
		return"~" === o.charAt(d) && (
			...
			c && f("~"), (
				b = f(/^"((?:[^"\\\r\n]|\\.)*)"|'((?:[^'\\\r\n]|\\.)*)'/)
				) ? new tree.Quoted(b[0], b[1] || b[2], c, e, a.currentFileInfo) : void 0
			) : void 0
	}, keyword: function () {
		var a;
		...
		return(
			...
			b = f(this.entity)
			) ? new tree.Assignment(a, b) : void 0
	}, url: function () {
		var b;
		...
		return"@" === o.charAt(p) && (
			b = f(/^@@?[\w-]+/)
			) ? new tree.Variable(b, c, a.currentFileInfo) : void 0
	}, variableCurly: function () {
		var b, c = p;
		return"@" === o.charAt(p) && (
			b = f(/^@\{([\w-]+)\}/)
			) ? new tree.Variable("@" + b[1], c, a.currentFileInfo) : void 0
	}, color: function () {
		var a;
		return"#" === o.charAt(p) && (
			a = f(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})/)
			) ? new tree.Color(a[1]) : void 0
	}, dimension: function () {
		...
		if (!(
			...
			return(
				a = f(/^([+-]?\d*\.?\d+)(%|[a-z]+)?/)
				) ? new tree.Dimension(a[1], a[2]) : void 0
		}
	}, unicodeDescriptor: function () {
		var a;
		return(
			a = f(/^U\+[0-9a-fA-F?]+(\-[0-9a-fA-F?]+)?/)
			) ? new tree.UnicodeDescriptor(a[0]) : void 0
	}, javascript: function () {
		var b, c, d = p;
		return"~" === o.charAt(d) && (
			d++, c = !0
			), "`" === o.charAt(d) ? (
			void 0 === a.javascriptEnabled || a.javascriptEnabled || i("You are using JavaScript, which has been disabled."), c && f("~"), (
				b = f(/^`([^`]*)`/)
				) ? new tree.JavaScript(b[1], p, c) : void 0
			) : void 0
	}}, variable: function () {
		var a;
		return"@" === o.charAt(p) && (
		...
		if ("." === l || "#" === l) {
			...
			return f("(") && (
				g = this.mixin.args.call(this, !0).args, h(")")
				), g = g || [], f(this.important) && (
				m = !0
				), i.length > 0 && (
				f(";") || j("}")
		...
		for (var b, c, d, e, g, j, k = [], l = [], m = [], n = {args: null, variadic: !1}; ;) {
			...
			var q = null;
			if (a ? 1 == j.value.length && (
				q = j.value[0]
				) : q = j, q && q instanceof tree.Variable) {
				if (f(":")) {
					k.length > 0 && (
						b && i("Cannot mix ; and , as delimiter types"), c = !0
						), g = h(this.expression), e = d = q.name;
				}
				else {
					if (!a && f(/^\.{3}/)) {
						n.variadic = !0, f(";") && !b && (
			...
			g && k.push(g), m.push({name: e, value: g}), f(",") || (
				...
				c && i("Cannot mix ; and , as delimiter types"), b = !0, k.length > 1 && (
					g = new tree.Value(k)
					), l.push({name: d, value: g}), d = null, k = [], c = !1
				)
		}
		...
		var a, d, e, g, i = [], k = !1;
		if (!(
			"." !== o.charAt(p) && "#" !== o.charAt(p) || j(/^[^{]*\}/)
			) && (
			b(), d = f(/^([#.](?:[\w-]|\\(?:[A-Fa-f0-9]{1,6} ?|[^A-Fa-f0-9]))+)\s*\(/)
			)) {
			a = d[1];
			var l = this.mixin.args.call(this, !1);
			if (i = l.args, k = l.variadic, f(")") || (
				t = p, c()
				), f(this.comments), f(/^when/) && (
				g = h(this.conditions, "expected condition")
				), e = f(this.block)) {
		...
		if (f(/^\(opacity=/i)) {
			return(
				a = f(/^\d+/) || f(this.entities.variable)
				) ? (
				h(")"), new tree.Alpha(a)
				) : void 0
		}
		...
		var b, c, d;
		return c = f(this.combinator), b = f(/^(?:\d+\.\d+|\d+)%/) || f(/^(?:[.#]?|:*)(?:[\w-]|[^\x00-\x9f]|\\(?:[A-Fa-f0-9]{1,6} ?|[^A-Fa-f0-9]))+/) || f("*") || f("&") || f(this.attribute) || f(/^\([^()@]+\)/) || f(/^[\.#](?=@)/) || f(this.entities.variableCurly), b || f("(") && (
			d = f(this.selector)
			) && f(")") && (
			b = new tree.Paren(d)
		...
		for (var c, d, e, g, j, k = [], l = []; (
			...
			g ? j = h(this.conditions, "expected condition") : j ? i("CSS guard can only be used at the end of selector") : e ? l.push.apply(l, e) : (
				l.length && i("Extend can only be used at the end of selector"), d = o.charAt(p), k.push(c), c = null
				), "{" !== d && "}" !== d && ";" !== d && "," !== d && ")" !== d
			);) {
			;
		}
		return k.length > 0 ? new tree.Selector(k, l, j, p, a.currentFileInfo) : (
			l.length && i("Extend must be used to extend a selector, it cannot be used on its own"), void 0
			)
	}, attribute: function () {
		var a, b, c;
		if (f("[")) {
			return(
				...
				b = f(this.entities.quoted) || f(/^[0-9]+%/) || f(/^[\w-]+/) || f(this.entities.variableCurly)
		...
		if (b(), "." !== i && "#" !== i && "&" !== i && (
			...
			if (g = d || !a.compress && "@" !== e.charAt(0) ? f(this.anonymousValue) || f(this.value) : f(this.value) || f(this.anonymousValue), h = f(this.important), "+" === e[e.length - 1] && (
		...
		var b, c, d = [];
		do if (b = f(this.entities.keyword) || f(this.entities.variable)) {
			d.push(b);
		}
		else if (f("(")) {
		...
		return a.dumpLineNumbers && (
			...
			b = f(this.mediaFeatures), c = f(this.block)
			) ? (
			d = new tree.Media(c, b, p, a.currentFileInfo), a.dumpLineNumbers && (
				d.debugInfo = e
				), d
			) : void 0
	}, directive: function () {
		...
		if ("@" === o.charAt(p)) {
			...
			if (b(), d = f(/^@[a-z-]+/)) {
				switch (h = d, "-" == d.charAt(1) && d.indexOf("-", 2) > 0 && (
		...
		return f(/^not/) && (
			...
			a = f(this.addition) || f(this.entities.keyword) || f(this.entities.quoted)
			) ? (
			(
				d = f(/^(?:>=|<=|=<|[<=>])/)
				) ? (
				b = f(this.addition) || f(this.entities.keyword) || f(this.entities.quoted)
				) ? c = new tree.Condition(d, a, b, e, g) : i("expected expression") : c = new tree.Condition("=", a, new tree.Keyword("true"), e, g), h(")"), f(/^and/) ? new tree.Condition("and", c, f(this.condition)) : c
			) : void 0
	}, operand: function () {
		...
		return a && (
			c.parensInOp = !0, c = new tree.Negative(c)
			), c
	}, expression: function () {
		for (var a, b, c = []; a = f(this.addition) || f(this.entity);) {
			c.push(a), !j(/^\/[\/*]/) && (
				b = f("/")
				) && c.push(new tree.Anonymous(b));
		}
	...
	a.functions = {rgb: function (a, b, c) {
		...
		function g(a) {
			return a = 0 > a ? a + 1 : a > 1 ? a - 1 : a, 1 > 6 * a ? i + 6 * (
				h - i
				) * a : 1 > 2 * a ? h : 2 > 3 * a ? i + 6 * (
				h - i
				) * (
		...
		d || (
			d = new a.Dimension(50)
			);
		var e = d.value / 100, f = 2 * e - 1, g = b.toHSL().a - c.toHSL().a, h = (
			(
				-1 == f * g ? f : (
					f + g
					) / (
			...
			) / 2, i = 1 - h, j = [b.rgb[0] * h + c.rgb[0] * i, b.rgb[1] * h + c.rgb[1] * i, b.rgb[2] * h + c.rgb[2] * i], k = b.alpha * e + c.alpha * (
			1 - e
			);
		...
		if (!(
			b instanceof a.Dimension
			)) {
			throw{type: "Argument", message: "the first argument to unit must be a number" + (
		...
		if ("number" == typeof b && "number" == typeof c) {
			...
			b instanceof a.Dimension && c instanceof a.Dimension
			)) {
		...
		for (d = 0; d < c.length; d++) {
			f = c[d], f instanceof a.Dimension ? (
				g = f.unify(), i = g.unit.toString(), e = k[i], void 0 !== e ? (
					h = j[e].unify(), (
		...
		return 1 == j.length ? j[0] : (
			c = j.map(function (a) {
				return a.toCSS(this.env)
			}).join(this.env.compress ? "," : ", "), new a.Anonymous((
				b ? "min" : "max"
				) + "(" + c + ")")
			)
	}, min: function () {
		...
		var c = a.rgb[0] < 128 ? 2 * a.rgb[0] * b.rgb[0] / 255 : 255 - 2 * (
			255 - a.rgb[0]
			) * (
			255 - b.rgb[0]
			) / 255, d = a.rgb[1] < 128 ? 2 * a.rgb[1] * b.rgb[1] / 255 : 255 - 2 * (
			255 - a.rgb[1]
			) * (
			255 - b.rgb[1]
			) / 255, e = a.rgb[2] < 128 ? 2 * a.rgb[2] * b.rgb[2] / 255 : 255 - 2 * (
			255 - a.rgb[2]
			) * (
			255 - b.rgb[2]
			) / 255;
		...
		var c = b.rgb[0] * a.rgb[0] / 255, d = c + a.rgb[0] * (
			255 - (
				255 - a.rgb[0]
				) * (
				255 - b.rgb[0]
		...
		var c = b.rgb[0] < 128 ? 2 * b.rgb[0] * a.rgb[0] / 255 : 255 - 2 * (
			255 - b.rgb[0]
			) * (
			255 - a.rgb[0]
			) / 255, d = b.rgb[1] < 128 ? 2 * b.rgb[1] * a.rgb[1] / 255 : 255 - 2 * (
			255 - b.rgb[1]
			) * (
			255 - a.rgb[1]
			) / 255, e = b.rgb[2] < 128 ? 2 * b.rgb[2] * a.rgb[2] / 255 : 255 - 2 * (
			255 - b.rgb[2]
			) * (
			255 - a.rgb[2]
			) / 255;
		...
		var c = a.rgb[0] + b.rgb[0] * (
			255 - a.rgb[0] - a.rgb[0]
			) / 255, d = a.rgb[1] + b.rgb[1] * (
			255 - a.rgb[1] - a.rgb[1]
			) / 255, e = a.rgb[2] + b.rgb[2] * (
			255 - a.rgb[2] - a.rgb[2]
			) / 255;
		return this.rgb(c, d, e)
		...
		if (arguments.length < 2 && (
			...
			e = this.currentFileInfo.relativeUrls ? g.join(this.currentFileInfo.currentDirectory, e) : g.join(this.currentFileInfo.entryPath, e)
			), arguments.length < 2) {
			var i;
			try {
				i = require("mime")
			...
			h = ["US-ASCII", "UTF-8"].indexOf(k) < 0, h && (
				d += ";base64"
				)
		}
		else {
		...
		for (e = '<?xml version="1.0" ?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" height="100%" viewBox="0 0 1 1" preserveAspectRatio="none"><' + l + 'Gradient id="gradient" gradientUnits="userSpaceOnUse" ' + d + ">", f = 0; f < k.length; f += 1) {
			k[f].value ? (
				...
				(
					0 === f || f + 1 === k.length
					) && void 0 === h || h instanceof a.Dimension
				) || c(), i = h ? h.toCSS(o) : 0 === f ? "0%" : "100%", j = g.alpha, e += '<stop offset="' + i + '" stop-color="' + g.toRGB() + '"' + (
				1 > j ? ' stop-opacity="' + j + '"' : ""
				) + "/>";
		}
		...
		return e = "'data:image/svg+xml" + (
			n ? ";base64" : ""
			) + "," + e + "'", new a.URL(new a.Anonymous(e))
	...
	a.Call = function (a, b, c, d) {
		...
		if (f in a.functions) {
			try {
				...
				throw{type: g.type || "Runtime", message: "error evaluating function `" + this.name + "`" + (
					g.message ? ": " + g.message : ""
	...
	a.Color.prototype = {type: "Color", eval: function () {
		...
		return"#" + this.rgb.map(function (a) {
			return a = Math.round(a), a = (
				a > 255 ? 255 : 0 > a ? 0 : a
				).toString(16), 1 === a.length ? "0" + a : a
		}).join("")
	}, toHSL: function () {
		var a, b, c = this.rgb[0] / 255, d = this.rgb[1] / 255, e = this.rgb[2] / 255, f = this.alpha, g = Math.max(c, d, e), h = Math.min(c, d, e), i = (
			g + h
			) / 2, j = g - h;
		if (g === h) {
			a = b = 0;
	...
	a.Dimension = function (b, c) {
		...
		if ("+" === c || "-" === c) {
			...
			"*" === c ? (
				f.numerator = f.numerator.concat(d.unit.numerator).sort(), f.denominator = f.denominator.concat(d.unit.denominator).sort(), f.cancel()
				) : "/" === c && (
				f.numerator = f.numerator.concat(d.unit.denominator).sort(), f.denominator = f.denominator.concat(d.unit.numerator).sort(), f.cancel()
				);
		}
	...
	}}, a.UnitConversions = {length: {m: 1, cm: .01, mm: .001, "in": .0254, pt: .0254 / 72, pc: 12 * (
		.0254 / 72
		)}, duration: {s: 1, ms: .001}, angle: {rad: 1 / (
		2 * Math.PI
		), deg: 1 / 360, grad: .0025, turn: 1}}, a.Unit = function (a, b, c) {
		...
		0 === this.numerator.length && 0 === this.denominator.length && c && (
			this.backupUnit = c
	...
	a.Directive = function (b, c, d, e) {
		...
		c.add(this.name, this.currentFileInfo, this.index), this.rules ? a.outputRuleset(b, c, this.rules) : (
			c.add(" "), this.value.genCSS(b, c), c.add(";")
			)
	}, toCSS: a.toCSS, eval: function (b) {
		var c = this;
		return this.rules && (
			b.frames.unshift(this), c = new a.Directive(this.name, null, this.index, this.currentFileInfo), c.rules = [this.rules[0].eval(b)], c.rules[0].root = !0, b.frames.shift()
			), c
	}, variable: function (b) {
	...
	a.Expression = function (a) {
		...
		return d && b.inParenthesis(), this.value.length > 1 ? c = new a.Expression(this.value.map(function (a) {
			...
			this.value[0].parens && !this.value[0].parensInOp && (
				e = !0
				), c = this.value[0].eval(b)
			) : c = this, d && b.outOfParenthesis(), this.parens && this.parensInOp && !b.isMathOn() && !e && (
			c = new a.Paren(c)
			), c
	}, genCSS: function (a, b) {
	...
	a.Extend = function (a, b, c) {
		...
		for (b = 0; b < a.length; b++) {
			c = a[b].elements, b > 0 && c.length && "" === c[0].combinator.value && (
	...
	a.Import = function (a, b, c, d, e) {
		...
		this.features = a.visit(this.features), this.path = a.visit(this.path), this.options.inline || (
	...
	a.Media = function (b, c, d, e) {
		...
		b.mediaBlocks || (
			b.mediaBlocks = [], b.mediaPath = []
			);
		var c = new a.Media([], [], this.index, this.currentFileInfo);
		this.debugInfo && (
			this.rules[0].debugInfo = this.debugInfo, c.debugInfo = this.debugInfo
			);
		var d = !1;
		b.strictMath || (
			d = !0, b.strictMath = !0
	...
	a.mixin = {}, a.mixin.Call = function (b, c, d, e, f) {
		...
		for (e = this.arguments && this.arguments.map(function (a) {
			...
			if ((
				c = b.frames[f].find(this.selector)
				).length > 0) {
				for (j = !0, g = 0; g < c.length; g++) {
					for (d = c[g], i = !1, h = 0; h < b.frames.length; h++) {
						if (!(
							...
							b.frames[h].originalRuleset || b.frames[h]
							)) {
							i = !0;
							break
						}
					...
					if (!i && d.matchArgs(e, b)) {
						if (!d.matchCondition || d.matchCondition(e, b)) {
							try {
								d instanceof a.mixin.Definition || (
									d = new a.mixin.Definition("", [], d.rules, null, !1), d.originalRuleset = c[g].originalRuleset || c[g]
									), Array.prototype.push.apply(l, d.eval(b, e, this.important).rules)
							} catch (n) {
		...
		throw j ? {type: "Runtime", message: "No matching definition was found for `" + this.selector.toCSS().trim() + "(" + (
			e ? e.map(function (a) {
				var b = "";
				return a.name && (
					b += a.name + ":"
			...
			) + ")`", index: this.index, filename: this.currentFileInfo.filename} : {type: "Name", message: this.selector.toCSS().trim() + " is undefined", index: this.index, filename: this.currentFileInfo.filename}
		...
		return i.rules.unshift(new a.Rule("@arguments", new a.Expression(g).eval(b))), e = this.rules.slice(0), f = new a.Ruleset(null, e), f.originalRuleset = this, f = f.eval(new a.evalEnv(b, [this, i].concat(h))), d && (
			f = this.parent.makeImportant.apply(f)
			), f
	...
	a.Rule = function (b, c, d, e, f, g, h) {
		...
		b.add(this.important + (
			this.inline || a.lastRule && a.compress ? "" : ";"
			), this.currentFileInfo, this.index)
	}, toCSS: a.toCSS, eval: function (b) {
		var c = !1;
		"font" !== this.name || b.strictMath || (
			c = !0, b.strictMath = !0
			);
		try {
	...
	a.Ruleset = function (a, b, c) {
		...
		for (g.originalRuleset = this, g.root = this.root, g.firstRoot = this.firstRoot, g.allowImports = this.allowImports, this.debugInfo && (
			g.debugInfo = this.debugInfo
			), b.frames.unshift(g), b.selectors || (
			b.selectors = []
			), b.selectors.unshift(this.selectors), (
			g.root || g.allowImports || !g.strictImports
			) && g.evalImports(b), e = 0; e < g.rules.length; e++) {
		...
		for (e = 0; e < g.rules.length; e++) {
			d = g.rules[e], d instanceof a.mixin.Definition || (
		...
		return this._variables ? this._variables : this._variables = this.rules.reduce(function (b, c) {
			return c instanceof a.Rule && c.variable === !0 && (
				b[c.name] = c
				), b
		}, {})
	}, variable: function (a) {
		...
		c = c || this;
		var d, e = [], f = b.toCSS();
		return f in this._lookups ? this._lookups[f] : (
		...
		if (!this.root) {
			for (f = a.debugInfo(b, this, m), f && (
				c.add(f), c.add(m)
				), d = 0; d < this.paths.length; d++) {
		...
		for (d = 0; d < i.length; d++) {
			g = i[d], d + 1 !== i.length || this.root && 0 !== j.length && !this.firstRoot || (
				b.lastRule = !0
			...
			for (r = [], i = [
				...
				if (j = d.elements[e], "&" !== j.value) {
					...
					for (s = [], r.length > 0 && this.mergeElementsOnToSelectors(r, i), f = 0; f < i.length; f++) {
						if (k = i[f], 0 === c.length) {
							k.length > 0 && (
								k[0].elements = k[0].elements.slice(0), k[0].elements.push(new a.Element(j.combinator, "", 0, j.index, j.currentFileInfo))
								), s.push(k);
						}
						else {
							for (g = 0; g < c.length; g++) {
								l = c[g], m = [], n = [], p = !0, k.length > 0 ? (
									m = k.slice(0), q = m.pop(), o = d.createDerived(q.elements.slice(0)), p = !1
									) : o = d.createDerived([]), l.length > 1 && (
									n = n.concat(l.slice(1))
									), l.length > 0 && (
									p = !1, o.elements.push(new a.Element(j.combinator, l[0].elements[0].value, j.index, j.currentFileInfo)), o.elements = o.elements.concat(l[0].elements.slice(1))
	...
	a.Selector = function (a, b, c, d, e, f) {
		this.elements = a, this.extendList = b || [], this.condition = c, this.currentFileInfo = e || {}, this.isReferenced = f, c || (
			this.evaldCondition = !0
			)
	...
	a.URL = function (a, b) {
		...
		return c = this.currentFileInfo && this.currentFileInfo.rootpath, c && "string" == typeof d.value && b.isPathRelative(d.value) && (
			d.quote || (
	...
	a.parseEnv = function (a) {
		if (d(a, this, b), this.contents || (
			...
			a && (
				a.filename = null
				), this.currentFileInfo = {filename: c, relativeUrls: this.relativeUrls, rootpath: a && a.rootpath || "", currentDirectory: e, entryPath: e, rootFilename: c}
		}
	};
	...
	a.visitor = function (a) {
		...
		return e && (
			b = {visitDeeper: !0}, c = e.call(this._implementation, a, b), this._implementation.isReplacing && (
				a = c
				)
			), (
			!b || b.visitDeeper
			) && a && a.accept && a.accept(this), d += "Out", this._implementation[d] && this._implementation[d](a), a
	...
	a.importVisitor = function (b, c, d) {
		...
		if (!b.css || f) {
			try {
				...
				g.filename || (
					g.index = b.index, g.filename = b.currentFileInfo.filename
					), b.css = !0, b.error = g
			}
			if (d && (
				!d.css || f
				...
				b.options.multiple && (
					...
					c && !c.filename && (
						c.index = b.index, c.filename = b.currentFileInfo.filename
						), g && !h.importMultiple && (
						b.skip = g
						);
					var j = function (a) {
	...
	a.joinSelectorVisitor = function () {
		...
		this.contexts.push(c), a.root || (
			...
			}), 0 === a.selectors.length && (
				a.rules.length = 0
				), a.joinSelectors(c, b, a.selectors), a.paths = c
			)
	}, visitRulesetOut: function () {
	...
	a.toCSSVisitor = function (b) {
		...
		if (b.firstRoot && this.checkPropertiesInRoot(b.rules), b.root) {
			b.accept(this._visitor), c.visitDeeper = !1, (
				b.firstRoot || b.rules.length > 0
				) && e.splice(0, 0, b);
		}
		else {
		...
		for (e = b.length - 1; e >= 0; e--) {
			if (d = b[e], d instanceof a.Rule) {
				if (f[d.name]) {
					c = f[d.name], c instanceof a.Rule && (
		...
		for (var c, d, e, f = {}, g = 0; g < b.length; g++) {
			d = b[g], d instanceof a.Rule && d.merge && (
				e = [d.name, d.important ? "!" : ""].join(","), f[e] ? b.splice(g--, 1) : c = f[e] = [], c.push(d)
				);
		}
		Object.keys(f).map(function (b) {
			c = f[b], c.length > 1 && (
				d = c[0], d.value = new a.Value(c.map(function (a) {
					return a.value
				}))
				)
		})
	...
	a.extendFinderVisitor = function () {
		...
		if (!b.root) {
			...
			for (c = 0; c < b.paths.length; c++) {
				...
				for (f = i.extendList.slice(0).concat(g).map(function (a) {
					...
					this.foundExtends = !0, e = f[d], e.findSelfSelectors(h), e.ruleset = b, 0 === d && (
						e.firstExtendOnThisSelectorPath = !0
		...
		return c.run(b), c.foundExtends ? (
			b.allExtends = b.allExtends.concat(this.doExtendChaining(b.allExtends, b.allExtends)), this.allExtendsStack = [b.allExtends], this._visitor.visit(b)
			) : b
		...
		for (d = d || 0, e = 0; e < b.length; e++) {
			for (f = 0; f < c.length; f++) {
				j = b[e], k = c[f], this.inInheritanceChain(k, j) || (
					i = [k.selfSelectors[0]], g = n.findMatch(j, i), g.length && j.selfSelectors.forEach(function (b) {
						h = n.extendSelector(g, i, b), l = new a.Extend(k.selector, k.option, 0), l.selfSelectors = h, h[h.length - 1].extendList = [l], m.push(l), l.ruleset = k.ruleset, l.parents = [k, j], k.firstExtendOnThisSelectorPath && (
		...
		if (!a.root) {
			...
			for (d = 0; d < f.length; d++) {
				for (c = 0; c < a.paths.length; c++) {
					e = a.paths[c], a.extendOnEveryPath || e[e.length - 1].extendList.length || (
						b = this.findMatch(f[d], e), b.length && f[d].selfSelectors.forEach(function (a) {
		...
		for (c = 0; c < b.length; c++) {
			for (d = b[c], e = 0; e < d.elements.length; e++) {
				for (f = d.elements[e], (
					...
					) && l.push({pathIndex: c, index: e, matched: 0, initialCombinator: f.combinator}), h = 0; h < l.length; h++) {
					i = l[h], g = f.combinator.value, "" === g && 0 === e && (
						g = " "
						), !j.isElementValuesEqual(k[i.matched].value, f.value) || i.matched > 0 && k[i.matched].combinator.value !== g ? i = null : i.matched++, i && (
						i.finished = i.matched === k.length, i.finished && !a.allowAfter && (
							e + 1 < d.elements.length || c + 1 < b.length
							) && (
							i = null
							)
						), i ? i.finished && (
						i.length = k.length, i.endPathIndex = c, i.endPathElementIndex = e + 1, l.length = 0, m.push(i)
						) : (
						l.splice(h, 1), h--
						);
				}
		...
		if (b instanceof a.Attribute) {
			return b.op !== c.op || b.key !== c.key ? !1 : b.value && c.value ? (
		...
		for (e = 0; e < b.length; e++) {
			h = b[e], f = c[h.pathIndex], g = new a.Element(h.initialCombinator, d.elements[0].value, d.elements[0].index, d.elements[0].currentFileInfo), h.pathIndex > j && k > 0 && (
				l[l.length - 1].elements = l[l.length - 1].elements.concat(c[j].elements.slice(k)), k = 0, j++
				), i = f.elements.slice(k, h.index).concat([g]).concat(d.elements.slice(1)), j === h.pathIndex && e > 0 ? l[l.length - 1].elements = l[l.length - 1].elements.concat(i) : (
				l = l.concat(c.slice(j, h.pathIndex)), l.push(new a.Selector(i))
				), j = h.endPathIndex, k = h.endPathElementIndex, k >= c[j].elements.length && (
				k = 0, j++
				);
		}
		return j < c.length && k > 0 && (
	...
	a.sourceMapOutput = function (a) {
		...
		if (a) {
			...
			1 === e.length ? this._column += g.length : (
		...
		if (this._rootNode.genCSS(a, this), this._css.length > 0) {
			...
			this._sourceMapFilename && (
				...
				), this._writeSourceMap ? this._writeSourceMap(d) : c = "data:application/json," + encodeURIComponent(d), c && this._css.push("/*# sourceMappingURL=" + c + " */")
		}
...
less.env = less.env || (
	"127.0.0.1" == location.hostname || "0.0.0.0" == location.hostname || "localhost" == location.hostname || location.port.length > 0 || isFileProtocol ? "development" : "production"
	);
var logLevel = {info: 2, errors: 1, none: 0};
if (less.logLevel = "undefined" != typeof less.logLevel ? less.logLevel : logLevel.info, less.async = less.async || !1, less.fileAsync = less.fileAsync || !1, less.poll = less.poll || (
	isFileProtocol ? 1e3 : 1500
	), less.functions) {
...
if (less.watch = function () {
	return less.watchMode || (
		less.env = "development", initRunningMode()
		), this.watchMode = !0
...
less.modifyVars = function (a) {
	...
	c = d = new Date, loadStyleSheets(function (a, b, e, f, g) {
		return a ? error(a, f.href) : (
			g.local ? log("loading " + f.href + " from cache.", logLevel.info) : (
				log("parsed " + f.href + " successfully.", logLevel.info), createCSS(b.toCSS(less), f, g.lastModified)
				), log("css for " + f.href + " generated in " + (
				new Date - d
				) + "ms", logLevel.info), 0 === g.remaining && log("css generated in " + (
				new Date - c
				) + "ms", logLevel.info), d = new Date, void 0
			)
	}, a, b), loadStyles(b)
